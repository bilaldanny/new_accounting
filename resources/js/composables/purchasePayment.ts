...
export default function usePurchasePayments(){
    ...
    const deleteRecord = async (ids: Array<number>) => {
        return deleteFn(API_ENDPOINTS.purchasePayments+'/bulk_delete', ids, state);
    }
    ...
    const getPurchasePayments = async (data: QueryParams) => {
        return getData(API_ENDPOINTS.purchasePayments, data, state)
    };
