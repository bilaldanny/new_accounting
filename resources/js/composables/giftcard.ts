...
export default function useGiftCards(){
    ...
    const changeStatus = async (ids: Array<number>, status: string) => {
        return changeStateFn(API_ENDPOINTS.giftCards+'/statusupdate',ids,status,state);
    }
    ...
    const deleteRecord = async (ids: Array<number>) => {
        return deleteFn(API_ENDPOINTS.giftCards+'/bulk_delete', ids, state);
    }

    const perDeleteBulkRecord = async (ids: Array<number>) => {
        return deleteFn(API_ENDPOINTS.giftCards+'/bulk_delete_per', ids, state);
    }
    ...
    const getGiftCards = async (data: QueryParams) => {
        return getData(API_ENDPOINTS.giftCards, data, state)
    };

    const getTrashGiftCards = async (data: QueryParams) => {
        return getData(API_ENDPOINTS.giftCards+'/trash', data, state);
    };
    ...
    const getEditData = async (id: number): Promise<boolean> => {
        ...
        try {
            const response = await fetchWithRetry(window.axios.get, `${API_ENDPOINTS.giftCards}/${id}`);
    ...
    const restoreBulkRecord = async (ids: Array<number>) => {
        return restoreFn(API_ENDPOINTS.giftCards+'/restore_records', ids, state)
    }
