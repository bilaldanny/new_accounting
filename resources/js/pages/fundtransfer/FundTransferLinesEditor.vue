<script setup lang="ts">
    ...
    import type { FundTransferLineRow } from '@/composables/fundtransfer';
    ...
    const props = defineProps({
        lines: {
            type: Array as () => FundTransferLineRow[],
            default: () => [],
        },
        accounts: {
    ...
    const emit = defineEmits<{
        add: [line: FundTransferLineRow];
        update: [index: number, line: FundTransferLineRow];
        remove: [index: number];
    }>();

    const draft = ref<FundTransferLineRow>({
        account_id: '',
        description: '',
        debit: '',
        credit: '',
    ...
    function applyAccount(line: FundTransferLineRow, accountId: number | string | ''): FundTransferLineRow {
    ...
    function duplicateLine(line: FundTransferLineRow) {
        if (props.disabled) {
            return;
        }
                ...
                <tbody>
                    <tr v-if="lines.length === 0 && editingIndex === null">
                        <td colspan="6" class="journal-lines__empty">
                            ...
                            <small>Add the "from" account as a credit and the "to" account as a debit (same amount) to balance this transfer.</small>
                        </td>
                    </tr>
                    <tr
