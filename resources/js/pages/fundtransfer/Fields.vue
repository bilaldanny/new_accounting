<script setup lang="ts">
    ...
    import { ArrowLeftRight, BookOpen, FileText, Paperclip, RefreshCw, UploadCloud } from '@lucide/vue';
    import { computed, onMounted, ref, watch } from 'vue';
    import useActiveFinancialYear from '@/composables/activeFinancialYear';
    import { API_ENDPOINTS } from '@/composables/apiEndpoints';
    import useCommons from '@/composables/common';
    import type { FundTransferAttachment, FundTransferLineRow } from '@/composables/fundtransfer';
    import FieldHint from './FieldHint.vue';
    import FundTransferLinesEditor from './FundTransferLinesEditor.vue';
    ...
    const transferLines = computed<FundTransferLineRow[]>(() => (
        Array.isArray(params.formData?.taccountdetails) ? params.formData.taccountdetails : []
    ));
    const attachments = computed<FundTransferAttachment[]>(() => (
        Array.isArray(params.formData?.attachments) ? params.formData.attachments.filter((file: FundTransferAttachment) => ! file.deleted) : []
    ));
    ...
    function persistLines(lines: FundTransferLineRow[]) {
    ...
    async function loadVoucherNo(force = false) {
        ...
        try {
            const response = await window.axios.get(API_ENDPOINTS.fundTransferVoucherNo, {
                params: {
    ...
    function addLine(line: FundTransferLineRow) {
        persistLines([...transferLines.value, line]);
    }

    function updateLine(index: number, line: FundTransferLineRow) {
        const next = [...transferLines.value];
        next[index] = line;
        persistLines(next);
    }
    ...
    function removeAttachment(index: number) {
        const next = [...(params.formData?.attachments ?? [])] as FundTransferAttachment[];
...
<template>
    ...
    <TextElement name="voucher_type" default="FT" hidden="true" rules="required" />
    ...
    <GroupElement name="group_voucher" :columns="colFull" :add-classes="cardClasses">
        ...
        >
            <template #label>
                <FieldHint label="Company" required>
                    Select the company this transfer belongs to.
                </FieldHint>
            </template>
        </SelectElement>
        ...
        >
            <template #label>
                <FieldHint label="Branch" required>
                    Required. Choose the branch where this transfer will be posted.
                </FieldHint>
            </template>
        </SelectElement>
        ...
        >
            <template #label>
                <FieldHint label="Voucher number">
                    ...
                    <span class="journal-field__tooltip-rule"><strong>FT:</strong> Fund transfer</span>
                </FieldHint>
            </template>
            <template #addon-before>
                <span class="journal-voucher-type journal-voucher-type--static">FT</span>
            </template>
        ...
        >
            <template #label>
                <FieldHint label="Transfer date" required>
        ...
        >
            <template #label>
                <FieldHint label="Reference number">
                    Optional. The bank transfer confirmation or UTR number for this transfer.
                </FieldHint>
            </template>
        </TextElement>
    </GroupElement>
                ...
                <div class="journal-card-head__lead">
                    ...
                    <div>
                        ...
                        <div class="journal-card-title-row">
                            <h2 class="product-form-section-title">From / to accounts</h2>
                            <span class="journal-panel__count">{{ lineCountLabel }}</span>
                        </div>
                    </div>
                </div>
                <button
                    v-if="transferLines.length > 0"
                    type="button"
    ...
    <GroupElement name="group_notes" :columns="colFull" :add-classes="cardClasses">
        ...
        <TextareaElement
            ...
            id="Comments"
            field-name="Comments"
            placeholder="Why funds are moving between these accounts…"
            :columns="colHalf"
            :rows="4"
        >
            <template #label>
        ...
        <StaticElement name="fundtransfer_attachments" :columns="colHalf">
            <div class="journal-field">
                <span class="journal-comments-label">
                    <FieldHint label="Attachments">
                        Optional. Bank advices or transfer slips can be attached to this voucher.
                    </FieldHint>
                ...
                <label
                    ...
                    for="fundtransfer-attachment-input"
                ...
                >
                    ...
                    <input
                        id="fundtransfer-attachment-input"
                        type="file"
                        multiple
