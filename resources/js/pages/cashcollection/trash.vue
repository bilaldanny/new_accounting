<script setup lang="ts">
    ...
    defineOptions({
        layout: {
            title: 'Cash Collection Trash',
            subtitle: 'Restore or permanently delete removed cash collections',
            breadcrumbs: [
                {
    ...
    const debouncedGetTrashCashCollections = debounce((params) => {
        getTrashCashCollections(params);
    }, 300);
    ...
    const getData = async () => {
        try {
            ...
            await debouncedGetTrashCashCollections({ ...state.search });
            ...
            console.error('Error fetching trash cash collections:', error);
        }
    };
    ...
    onMounted(async () => {
        ...
        debouncedGetTrashCashCollections({ ...state.search });
    });
    ...
    const fetchAllRowsForExport = createTableExportAllRows(`${API_ENDPOINTS.cashCollections}/trash`, () => state);
</script>
