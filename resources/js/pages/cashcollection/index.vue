<script setup lang="ts">
    ...
    defineOptions({
        layout: {
            ...
            subtitle: 'Cash collected from customers and posted against their invoices',
            breadcrumbs: [
                {
                    title: 'Cash Collection',
                    href: 'NULL',
    ...
    const columns = computed(() => [
        ...
        { key: 'contact_name', label: 'Customer', type: 'secondary', responsive: ['xs', 'sm', 'md', 'lg'], emptyDisplay: '-', sorting: 'disabled' },
        { key: 'branch_name', label: 'Branch', type: 'secondary', responsive: ['md', 'lg'], emptyDisplay: '-', sorting: 'disabled' },
        { key: 'collected_on', label: 'Collected On', type: 'secondary', responsive: ['sm', 'md', 'lg'] },
        { key: 'amount', label: 'Amount', type: 'secondary', responsive: ['xs', 'sm', 'md', 'lg'] },
    ...
    const debouncedGetCashCollections = debounce((params) => {
        getCashCollections(params);
    }, 300);
    ...
    const getData = async () => {
        try {
            ...
            await debouncedGetCashCollections({ ...state.search });
    ...
    onMounted(async () => {
        ...
        debouncedGetCashCollections({ ...state.search });
    });
    ...
    const fetchAllRowsForExport = createTableExportAllRows(API_ENDPOINTS.cashCollections, () => state);
        ...
        <div class="admin-list-card">
            ...
            <TheFilter v-if="showCompanyFilter" v-model:open="filterOpen" :loading="state.loading" @clear="clearSearch" @search="getData">
                <div class="col-md-4 col-lg-3 admin-filter-field">
                    <label class="form-label" for="cashcollection-filter-company">Company</label>
                    <select
                        id="cashcollection-filter-company"
            ...
            <div class="admin-list-card__body">
                <div class="admin-list-table">
                    <TheTable
                        ...
                        :delete="deleteRecord"
                        actionType="link"
                        :viewRoute="(id: number) => `/cashcollection/${id}/view`"
