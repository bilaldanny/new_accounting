<script setup lang="ts">
    ...
    const modalProps = defineProps({
        ...
        sampleFileName: { type: String, default: 'brand-import-sample.xlsx' },
    });
    ...
    const fallbackSampleRows = [['', 'Sample Brand', 1]];

    function mapBrandToSampleRow(brand: Record<string, unknown>): (string | number)[] {
        return [
            normalizeImportExportValue(brand.id),
            normalizeImportExportValue(brand.name),
            normalizeImportBool(brand.active),
        ];
    }
    ...
    async function downloadSampleFile() {
        ...
        try {
            await downloadImportTemplate({
                ...
                listApiUrl: API_ENDPOINTS.brands,
                listQuery: { sort_type: 'desc' },
                mapRow: mapBrandToSampleRow,
    ...
    async function submitImport() {
        ...
        try {
            ...
            if (rows.length === 0) {
                fileError.value = 'The selected file does not contain any brand rows.';

                return;
            }

            const response = await window.axios.post(API_ENDPOINTS.brandImport, { rows });
...
<template>
    <ImportModalLayout
        ...
        file-input-id="brand-import-file"
