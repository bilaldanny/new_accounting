<script setup lang="ts">
    ...
    defineOptions({
        layout: {
            ...
            subtitle: 'Manage customer pricing groups',
            breadcrumbs: [
                {
                    title: 'Customer Group Management',
                    href: 'NULL',
    ...
    const columns = computed(() => [
        ...
        { key: 'name', label: 'Customer Group Name', type: 'primary', responsive: ['xs', 'sm', 'md', 'lg'] },
    ...
    const debouncedGetCustomerGroups = debounce((params) => {
    ...
    const getData = async () => {
        try {
            ...
            await debouncedGetCustomerGroups({ ...state.search });
    ...
    onMounted(async () => {
        ...
        debouncedGetCustomerGroups({ ...state.search });
    });
    ...
    const fetchAllRowsForExport = createTableExportAllRows(API_ENDPOINTS.customerGroups, () => state);
...
<template>
        ...
        <div class="admin-list-card">
            ...
            <TheFilter v-if="showFilter" v-model:open="filterOpen" :loading="state.loading" @clear="clearSearch" @search="getData">
                <div v-if="showCompanyFilter" class="col-md-4 col-lg-3 admin-filter-field">
                    <label class="form-label" for="customer-group-filter-company">Company</label>
                    <select
                        id="customer-group-filter-company"
                ...
                <div v-if="showBranchFilter" class="col-md-4 col-lg-3 admin-filter-field">
                    <label class="form-label" for="customer-group-filter-branch">Branch</label>
                    <select
                        id="customer-group-filter-branch"
                        class="form-select form-select-sm"
    ...
    <AddModal
        ...
        :formData="formData"
        :formRef="form$"
        :endpoint="API_ENDPOINTS.customerGroups"
        :onOpen="openAddModal"
    ...
    <EditModal
        ...
        :endpoint="`${API_ENDPOINTS.customerGroups}/${edit_id.id}`"
