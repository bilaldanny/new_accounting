<script setup lang="ts">
    ...
    defineOptions({
        layout: {
            title: 'Currency Trash',
            subtitle: 'Restore or permanently delete removed currencies',
            breadcrumbs: [
                {
    ...
    const debouncedGetTrashCurrencies = debounce((params) => {
        getTrashCurrencies(params);
    }, 300);
    ...
    const getData = async () => {
        try {
            ...
            await debouncedGetTrashCurrencies({ ...state.search });
    ...
    onMounted(() => {
        ...
        debouncedGetTrashCurrencies({ ...state.search });
    });

    const fetchAllRowsForExport = createTableExportAllRows(`${API_ENDPOINTS.currencies}/trash`, () => state);
</script>
