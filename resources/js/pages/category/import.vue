<script setup lang="ts">
    ...
    const modalProps = defineProps({
        ...
        sampleFileName: { type: String, default: 'category-import-sample.xlsx' },
    });
    ...
    const sampleColumns = ['id', 'name', 'parent', 'active'] as const;
    const fallbackSampleRows = [['', 'Electronics', '', 1]];

    function mapCategoryToSampleRow(category: Record<string, unknown>): (string | number)[] {
        return [
            normalizeImportExportValue(category.id),
            normalizeImportExportValue(category.name),
            normalizeImportExportValue(category.parent_name ?? category.parent_id),
            normalizeImportBool(category.active),
        ];
    }
    ...
    async function downloadSampleFile() {
        ...
        try {
            await downloadImportTemplate({
                ...
                listApiUrl: API_ENDPOINTS.categories,
                listQuery: { sort_type: 'desc' },
                mapRow: mapCategoryToSampleRow,
    ...
    async function submitImport() {
        ...
        try {
            ...
            if (rows.length === 0) {
                fileError.value = 'The selected file does not contain any category rows.';

                return;
            }

            const response = await window.axios.post(API_ENDPOINTS.categoryImport, { rows });
...
<template>
    <ImportModalLayout
        ...
        file-input-id="category-import-file"
        ...
        secondary-text="The template includes existing records for updates. Leave id blank to add new rows. Parent accepts a category name or id."
