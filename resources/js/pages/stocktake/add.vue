<script setup lang="ts">
    ...
    import StockTakeFormChrome from './StockTakeFormChrome.vue';

    defineOptions({
        layout: {
            title: 'New Stock Take',
            subtitle: 'Open a count sheet for a branch',
            breadcrumbs: [
                {
                    title: 'Stock Take',
                    href: '/stocktake',
    ...
    const { formData, defaultFormData, emptyForm } = useStockTakes();
    ...
    async function submitStockTake(form$: { data?: Record<string, unknown> }) {
        const response = await window.axios.post(API_ENDPOINTS.stockTakes, {
            ...form$?.data,
        });
        ...
        await router.visit(response.data?.id ? `/stocktake/${response.data.id}/view` : '/stocktake');
...
<template>
    <Head :title="`New ${formatedText('stocktake')}`" />
        ...
        >
            <div class="product-form product-form--sectioned purchase-form">
                ...
                <TheForm
                    ...
                    :key="formKey"
                    :onSubmit="submitStockTake"
                    :formData="formData"
                    :error="handleFormError"
                    :url="API_ENDPOINTS.stockTakes"
                    ref="formRef"
                >
