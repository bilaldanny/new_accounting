<script setup lang="ts">
    ...
    defineOptions({
        layout: {
            title: 'Bank Issuer',
            subtitle: 'Manage the external banks used to receive customer cheques',
            breadcrumbs: [
                {
                    title: 'Bank Issuer',
                    href: 'NULL',
    ...
    const columns = computed(() => [
        ...
        { key: 'name', label: 'Bank Issuer', type: 'primary', responsive: ['xs', 'sm', 'md', 'lg'] },
    ...
    const debouncedGetBankIssuers = debounce((params) => {
        getBankIssuers(params);
    }, 300);
    ...
    const getData = async () => {
        try {
            ...
            await debouncedGetBankIssuers({ ...state.search });
            ...
            console.error('Error fetching bank issuers:', error);
        }
    };
    ...
    onMounted(async () => {
        ...
        debouncedGetBankIssuers({ ...state.search });
    });
    ...
    const fetchAllRowsForExport = createTableExportAllRows(API_ENDPOINTS.bankIssuers, () => state);
        ...
        <div class="admin-list-card">
            ...
            <TheFilter v-if="showCompanyFilter" v-model:open="filterOpen" :loading="state.loading" @clear="clearSearch" @search="getData">
                <div class="col-md-4 col-lg-3 admin-filter-field">
                    <label class="form-label" for="bankissuer-filter-company">Company</label>
                    <select
                        id="bankissuer-filter-company"
