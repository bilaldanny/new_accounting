<script setup lang="ts">
    ...
    defineOptions({
        layout: {
            title: 'Bank Issuer Trash',
            subtitle: 'Restore or permanently delete removed bank issuers',
            breadcrumbs: [
                {
                    title: 'Bank Issuer',
                    href: '/bankissuer',
    ...
    const debouncedGetTrashBankIssuers = debounce((params) => {
        getTrashBankIssuers(params);
    }, 300);
    ...
    const getData = async () => {
        try {
            ...
            await debouncedGetTrashBankIssuers({ ...state.search });
            ...
            console.error('Error fetching trash bank issuers:', error);
        }
    };
    ...
    onMounted(async () => {
        ...
        debouncedGetTrashBankIssuers({ ...state.search });
    });
    ...
    const fetchAllRowsForExport = createTableExportAllRows(`${API_ENDPOINTS.bankIssuers}/trash`, () => state);
</script>
