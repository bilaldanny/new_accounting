<script setup lang="ts">
    ...
    function mapCompanyToSampleRow(company: Record<string, unknown>): (string | number)[] {
        return [
            normalizeImportExportValue(company.id),
            normalizeImportExportValue(company.code),
            normalizeImportExportValue(company.name),
            normalizeImportExportValue(company.email),
            normalizeImportExportValue(company.phone),
            normalizeImportExportValue(company.ntn_no),
            normalizeImportExportValue(company.address),
            normalizeImportExportValue(company.zipcode),
            normalizeImportExportValue(company.country ?? company.country_name ?? company.country_id),
            normalizeImportExportValue(company.state ?? company.state_name ?? company.state_id),
            normalizeImportExportValue(company.city ?? company.city_name ?? company.city_id),
            normalizeImportExportValue(company.max_users),
            normalizeImportExportValue(company.max_branches),
            normalizeImportBool(company.is_active),
            normalizeImportExportValue(company.admin_name),
            normalizeImportExportValue(company.admin_username),
            normalizeImportExportValue(company.admin_email),
            normalizeImportExportValue(company.admin_phone),
            '',
        ];
    }
    ...
    async function downloadSampleFile() {
        ...
        try {
            await downloadImportTemplate({
                ...
                listApiUrl: API_ENDPOINTS.companies,
                listQuery: { sort_type: 'desc' },
                mapRow: mapCompanyToSampleRow,
    ...
    async function submitImport() {
        ...
        try {
            ...
            if (rows.length === 0) {
                fileError.value = 'The selected file does not contain any company rows.';
...
<template>
    <ImportModalLayout
        ...
        file-input-id="company-import-file"
        :selected-file="selectedFile"
        :file-error="fileError"
        :is-downloading-sample="isDownloadingSample"
        :is-importing="isImporting"
        :import-result="importResult"
        lead-text="Company name and admin details are required on every new row."
