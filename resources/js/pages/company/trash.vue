<script setup lang="ts">
    ...
    defineOptions({
        layout: {
            title: 'Company Trash',
            subtitle: 'Restore or permanently delete removed companies',
            breadcrumbs: [
                {
    ...
    const debouncedGetTrashCompanies = debounce((params) => {
        getTrashCompanies(params);
    }, 300);
    ...
    const getData = async () => {
        try {
            ...
            await debouncedGetTrashCompanies({ ...state.search });
    ...
    onMounted(() => {
        ...
        debouncedGetTrashCompanies({ ...state.search });
    });

    const fetchAllRowsForExport = createTableExportAllRows(`${API_ENDPOINTS.companies}/trash`, () => state);
