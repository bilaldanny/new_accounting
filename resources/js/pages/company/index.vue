<script setup lang="ts">
    ...
    import useCompanies from '@/composables/company';
    ...
    const columns = [
        ...
        { key: 'name', label: 'Company Name', type: 'primary', linkable: true, responsive: ['xs', 'sm', 'md', 'lg'] },
        { key: 'email', label: 'Company Email', type: 'secondary', responsive: ['md', 'lg'], emptyDisplay: '-' },
        { key: 'phone', label: 'Company Phone', type: 'secondary', responsive: ['lg'], emptyDisplay: '-' },
        { key: 'admin_name', label: 'Owner Name', type: 'secondary', responsive: ['md', 'lg'], emptyDisplay: '-' },
        { key: 'admin_email', label: 'Owner Email', type: 'secondary', responsive: ['lg'], emptyDisplay: '-' },
        { key: 'admin_phone', label: 'Owner Phone', type: 'secondary', responsive: ['lg'], emptyDisplay: '-' },
        { key: 'branches_usage', label: 'Branches (Created/Allowed)', type: 'secondary', responsive: ['md', 'lg'], sorting: 'disabled', emptyDisplay: '0/0' },
        { key: 'users_usage', label: 'Users (Created/Allowed)', type: 'secondary', responsive: ['md', 'lg'], sorting: 'disabled', emptyDisplay: '0/0' },
    ...
    const debouncedGetCompanies = debounce((params) => {
        getCompanies(params);
    }, 300);
    ...
    const getData = async () => {
        try {
            ...
            await debouncedGetCompanies({ ...state.search });
    ...
    onMounted(() => {
        ...
        debouncedGetCompanies({ ...state.search });
    });
    ...
    const openAddModal = async () => {
        ...
        formData.value.code = await generateCompanyCode();
        state.modalLoading = false;
    };
    ...
    const fetchAllRowsForExport = createTableExportAllRows(API_ENDPOINTS.companies, () => state);
...
<template>
        ...
        <div class="admin-list-card">
            ...
            <TheFilter v-model:open="filterOpen" :loading="state.loading" @clear="clearSearch" @search="getData">
                <div class="col-md-4 col-lg-3 admin-filter-field">
                    <label class="form-label" for="company-filter-status">Status</label>
                    <select
                ...
                <div class="col-md-4 col-lg-3 admin-filter-field">
                    <label class="form-label" for="company-filter-records">Show records</label>
                    <select
                        id="company-filter-records"
                        class="form-select form-select-sm"
    ...
    <EditModal
        ...
        :formData="formData"
        :formRef="form$"
        :endpoint="`${API_ENDPOINTS.companies}/${edit_id.id}`"
        :onClose="handleEditModalClose"
        :success="(response) => handleSuccess(response, form$)"
        :error="(error, details) => handleError(error, details, form$)"
        :logo-url="formData.logo_url"
        :record-id="edit_id.id"
    />
