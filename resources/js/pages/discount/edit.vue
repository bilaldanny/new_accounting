<script setup lang="ts">
    ...
    import DiscountFormChrome from './DiscountFormChrome.vue';
    ...
    defineOptions({
        layout: {
            title: 'Edit Discount',
            subtitle: 'Update the discount rule and status',
            breadcrumbs: [
                {
                    title: 'Discount',
                    href: '/discount',
    ...
    const { formData, getEditData } = useDiscounts();
    ...
    const endpoint = computed(() => `${API_ENDPOINTS.discounts}/${recordId.value}`);

    function handleFormError(error: unknown, details?: unknown) {
        handleError(error, details, formRef);
    }

    async function submitDiscount(form$: { data?: Record<string, unknown> }) {
        ...
        await router.visit(saveAction.value === 'add-new' ? '/discount/add' : '/discount');
...
<template>
    <Head :title="`Edit ${formatedText('discount')}`" />
    ...
    <div class="product-form-page purchase-form-page">
        <DiscountFormChrome
            ...
            @cancel="router.visit('/discount')"
            @save="save"
        >
            <div class="product-form product-form--sectioned purchase-form">
                <Loader v-if="!pageReady" message="Loading discounts…" />
