<script setup lang="ts">
    ...
    import useDiscounts from '@/composables/discount';
    ...
    defineOptions({
        layout: {
            title: 'Discount',
            subtitle: 'Manage coupon and discount rules for sales',
            breadcrumbs: [
                {
                    title: 'Discount',
                    href: 'NULL',
    ...
    const columns = computed(() => [
        ...
        { key: 'name', label: 'Discount', type: 'primary', responsive: ['xs', 'sm', 'md', 'lg'] },
        { key: 'code', label: 'Code', type: 'secondary', responsive: ['xs', 'sm', 'md', 'lg'], emptyDisplay: '-' },
        { key: 'discount_type', label: 'Type', type: 'secondary', responsive: ['sm', 'md', 'lg'], emptyDisplay: '-' },
        { key: 'value', label: 'Value', type: 'secondary', responsive: ['sm', 'md', 'lg'], emptyDisplay: '-' },
        { key: 'min_purchase_amount', label: 'Min Purchase', type: 'secondary', responsive: ['md', 'lg'], emptyDisplay: '-' },
        { key: 'expires_at', label: 'Expires', type: 'secondary', responsive: ['md', 'lg'], emptyDisplay: 'No expiry' },
    ...
    const debouncedGetDiscounts = debounce((params) => {
        getDiscounts(params);
    }, 300);
    ...
    const getData = async () => {
        try {
            ...
            await debouncedGetDiscounts({ ...state.search });
    ...
    onMounted(async () => {
        ...
        debouncedGetDiscounts({ ...state.search });
    });
    ...
    const fetchAllRowsForExport = createTableExportAllRows(API_ENDPOINTS.discounts, () => state);
        ...
        <div class="admin-list-card">
            ...
            <TheFilter v-if="showCompanyFilter" v-model:open="filterOpen" :loading="state.loading" @clear="clearSearch" @search="getData">
                <div class="col-md-4 col-lg-3 admin-filter-field">
                    <label class="form-label" for="discount-filter-company">Company</label>
                    <select
                        id="discount-filter-company"
            ...
            <div class="admin-list-card__body">
                <div class="admin-list-table">
                    <TheTable
                        ...
                        :delete="deleteRecord"
                        actionType="link"
                        :viewRoute="(id: number) => `/discount/${id}/view`"
