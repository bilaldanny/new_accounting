<script setup lang="ts">
    ...
    defineOptions({
        layout: {
            title: 'Deposit',
            subtitle: 'Bank, cash, and online receipts',
            breadcrumbs: [
                {
                    title: 'Deposit',
                    href: 'NULL',
    ...
    const debouncedGetDeposits = debounce((params) => {
        getDeposits(params);
    }, 300);
    ...
    const getData = async () => {
        try {
            ...
            await debouncedGetDeposits({ ...state.search });
    ...
    onMounted(async () => {
        ...
        debouncedGetDeposits({ ...state.search });
    });
    ...
    const fetchAllRowsForExport = createTableExportAllRows(API_ENDPOINTS.deposits, () => state);
        ...
        <div class="admin-list-card">
            ...
            <TheFilter v-if="showFilter" v-model:open="filterOpen" :loading="state.loading" @clear="clearSearch" @search="getData">
                <div v-if="showCompanyFilter" class="col-md-4 col-lg-3 admin-filter-field">
                    <label class="form-label" for="deposit-filter-company">Company</label>
                    <select
                        id="deposit-filter-company"
                ...
                <div v-if="showBranchFilter" class="col-md-4 col-lg-3 admin-filter-field">
                    <label class="form-label" for="deposit-filter-branch">Branch</label>
                    <select
                        id="deposit-filter-branch"
                        class="form-select form-select-sm"
                ...
                <div class="col-md-4 col-lg-3 admin-filter-field">
                    <label class="form-label" for="deposit-filter-status">Status</label>
                    <select id="deposit-filter-status" class="form-select form-select-sm" v-model="state.search.status">
