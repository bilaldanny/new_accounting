<script setup lang="ts">
    ...
    defineOptions({
        layout: {
            title: 'Bank Trash',
            subtitle: 'Restore or permanently delete removed banks',
            breadcrumbs: [
                {
    ...
    const columns = [
        ...
        { key: 'bank_name', label: 'Bank Name', type: 'primary', responsive: ['sm', 'md', 'lg'] },
    ...
    const debouncedGetTrashBanks = debounce((params) => {
        getTrashBanks(params);
    }, 300);
    ...
    const getData = async () => {
        try {
            ...
            await debouncedGetTrashBanks({ ...state.search });
            ...
            console.error('Error fetching bank trash:', error);
        }
    };
    ...
    onMounted(async () => {
        ...
        debouncedGetTrashBanks({ ...state.search });
    });
    ...
    const fetchAllRowsForExport = createTableExportAllRows(`${API_ENDPOINTS.banks}/trash`, () => state);
</script>
