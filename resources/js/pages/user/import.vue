<script setup lang="ts">
    ...
    function mapUserToSampleRow(user: Record<string, unknown>): (string | number)[] {
        return [
            normalizeImportExportValue(user.id),
            normalizeImportExportValue(user.branch_id),
            normalizeImportExportValue(user.department_id),
            normalizeImportExportValue(user.role_id),
            normalizeImportExportValue(user.first_name),
            normalizeImportExportValue(user.last_name),
            normalizeImportExportValue(user.username),
            normalizeImportExportValue(user.email),
            '',
            normalizeImportExportValue(user.phone),
            normalizeImportExportValue(user.user_image),
            normalizeImportBool(user.is_active),
        ];
    }
    ...
    async function downloadSampleFile() {
        ...
        try {
            await downloadImportTemplate({
                ...
                listApiUrl: API_ENDPOINTS.users,
                listQuery: { sort_type: 'desc' },
                mapRow: mapUserToSampleRow,
    ...
    async function submitImport() {
        ...
        try {
            ...
            if (rows.length === 0) {
                fileError.value = 'The selected file does not contain any user rows.';
...
<template>
    <ImportModalLayout
        ...
        file-input-id="user-import-file"
        :selected-file="selectedFile"
        :file-error="fileError"
        :is-downloading-sample="isDownloadingSample"
        :is-importing="isImporting"
        :import-result="importResult"
        lead-text="First name, username, and email are required on every row."
        secondary-text="The template includes existing records for updates. Leave id blank to add new users."
