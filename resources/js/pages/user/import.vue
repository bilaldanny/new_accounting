<script setup lang="ts">
    ...
    const modalProps = defineProps({
        ...
        sampleFileName: { type: String, default: 'user-import-sample.xlsx' },
    });
    ...
    const fallbackSampleRows = [
        ['', '', '', '', 2, 'John', 'Doe', 'johndoe', 'john@example.com', 'Password1!', '', '/uploads/users/default.png', 1],
    ];
    ...
    function mapUserToSampleRow(user: Record<string, unknown>): (string | number)[] {
        return [
            normalizeExportValue(user.id),
            normalizeExportValue(user.company_id),
            normalizeExportValue(user.branch_id),
            normalizeExportValue(user.department_id),
            normalizeExportValue(user.role_id),
            normalizeExportValue(user.first_name),
            normalizeExportValue(user.last_name),
            normalizeExportValue(user.username),
            normalizeExportValue(user.email),
            '',
            normalizeExportValue(user.phone),
            normalizeExportValue(user.user_image),
    ...
    async function fetchAllUserRows(): Promise<Record<string, unknown>[]> {
        const params = {
            status: 'all',
            search: '',
            sort_by: 'created_at',
        ...
        const { data: body } = await window.axios.get(API_ENDPOINTS.users, { params });
        ...
        for (let page = 2; page <= lastPage; page++) {
            const { data: nextBody } = await window.axios.get(API_ENDPOINTS.users, {
    ...
    async function submitImport() {
        ...
        try {
            ...
            if (rows.length === 0) {
                Notify('The Excel file does not contain any user rows.', 'alert');

                return;
            }

            const response = await window.axios.post(API_ENDPOINTS.userImport, { rows });
...
<template>
    ...
    >
        <div class="row g-3">
                ...
                <div class="alert alert-light border d-flex flex-column flex-sm-row align-items-sm-center justify-content-between gap-3 mb-0">
                    <div>
                        ...
                        <div class="text-muted small">
                            Download the sample Excel file with existing user records when available. Leave the id column empty to add a new user, or keep the id to update an existing record.
                        </div>
                    </div>
            ...
            <div class="col-12">
                <label class="form-label" for="user-import-file">Excel file</label>
                <input
                    id="user-import-file"
                    ref="fileInput"
                    type="file"
