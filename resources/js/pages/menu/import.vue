<script setup lang="ts">
    ...
    function mapMenuToSampleRow(menu: Record<string, unknown>): (string | number)[] {
        return [
            normalizeExportValue(menu.id),
            normalizeExportValue(menu.parent_id),
            normalizeExportValue(menu.type),
            normalizeExportValue(menu.name),
            normalizeExportValue(menu.route_path),
            normalizeExportValue(menu.route_name),
            normalizeExportValue(menu.icon),
            normalizeExportValue(menu.sort_order ?? 0),
            normalizeBool(menu.is_active),
            normalizeBool(menu.is_hidden),
            normalizeBool(menu.is_admin),
            normalizeBool(menu.is_permission),
        ];
    }
    ...
    async function fetchAllMenuRows(): Promise<Record<string, unknown>[]> {
        const params = {
            status: 'all',
            search: '',
            sort_by: 'sort_order',
        ...
        const { data: body } = await window.axios.get(API_ENDPOINTS.menus, { params });
        ...
        for (let page = 2; page <= lastPage; page++) {
            const { data: nextBody } = await window.axios.get(API_ENDPOINTS.menus, {
    ...
    async function downloadSampleFile() {
        ...
        try {
            const menus = await fetchAllMenuRows();
            const rows = menus.length > 0
                ? menus.map(mapMenuToSampleRow)
                : fallbackSampleRows;
    ...
    async function submitImport() {
        ...
        try {
            ...
            if (rows.length === 0) {
                Notify('The Excel file does not contain any menu rows.', 'alert');
...
<template>
    ...
    >
        <div class="row g-3">
                ...
                <div class="alert alert-light border d-flex flex-column flex-sm-row align-items-sm-center justify-content-between gap-3 mb-0">
                    <div>
                        ...
                        <div class="text-muted small">
                            Download the sample Excel file with existing menu records when available. Leave the id column empty to add a new menu, or keep the id to update an existing record.
                        </div>
                    </div>
            ...
            <div class="col-12">
                <label class="form-label" for="menu-import-file">Excel file</label>
                <input
                    id="menu-import-file"
                    ref="fileInput"
                    type="file"
