<script setup lang="ts">
    ...
    defineOptions({
        layout: {
            ...
            subtitle: 'Update branches, items, and transfer status',
            breadcrumbs: [
                {
    ...
    const { formData, getEditData } = useStockTransfers();
    ...
    const endpoint = computed(() => `${API_ENDPOINTS.stockTransfers}/${recordId.value}`);
    ...
    async function submitWithLines(form$: { data?: Record<string, unknown> }) {
        ...
        await router.visit(saveAction.value === 'add-new' ? '/stocktransfer/add' : '/stocktransfer');
...
<template>
    <Head :title="`Edit ${formatedText('stocktransfer')}`" />
    ...
    <div class="product-form-page purchase-form-page">
        ...
        >
            <div class="product-form product-form--sectioned purchase-form">
                <Loader v-if="!pageReady" message="Loading transfer…" />
