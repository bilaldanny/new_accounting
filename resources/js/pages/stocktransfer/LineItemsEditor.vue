<script setup lang="ts">
    ...
    const props = defineProps({
        lines: {
            type: Array as () => StockTransferLineRow[],
            default: () => [],
        },
        suggestions: {
    ...
    const emit = defineEmits<{
        ...
        update: [index: number, patch: Partial<StockTransferLineRow>];
        remove: [index: number];
    }>();
    ...
    function packingLabel(line: StockTransferLineRow): string {
        return line.unit_name || line.units?.find((unit) => String(unit.id) === String(line.unit_id))?.short_name || '';
    }

    function lineExceedsStock(line: StockTransferLineRow): boolean {
    ...
    <div class="purchase-lines">
        <div v-if="isSearchBox" class="purchase-lines__search" :class="{ 'is-disabled': disabled, 'is-open': open && !disabled }">
            ...
            <input
                ...
                :placeholder="disabled ? 'Select company and source branch first' : 'Search name, SKU, or barcode to add a product'"
                :value="query"
                :disabled="disabled"
                autocomplete="off"
                @input="onSearch"
        ...
        <div v-if="!hasLines" class="purchase-lines__empty">
            ...
            <strong>No products added to this transfer</strong>
            <span>{{ disabled
                ? 'Choose a company and source branch, then add products.'
                : isSearchBox
...
.purchase-lines__stock-pill.is-danger {
