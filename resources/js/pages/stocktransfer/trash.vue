<script setup lang="ts">
    ...
    defineOptions({
        layout: {
            title: 'Stock Transfer Trash',
            subtitle: 'Restore or permanently delete removed stock transfers',
            breadcrumbs: [
                {
    ...
    const debouncedGetTrashStockTransfers = debounce((params) => {
        getTrashStockTransfers(params);
    }, 300);
    ...
    const getData = async () => {
        try {
            ...
            await debouncedGetTrashStockTransfers({ ...state.search });
            ...
            console.error('Error fetching trash stock transfers:', error);
        }
    };
    ...
    onMounted(async () => {
        ...
        debouncedGetTrashStockTransfers({ ...state.search });
    });
    ...
    const fetchAllRowsForExport = createTableExportAllRows(`${API_ENDPOINTS.stockTransfers}/trash`, () => state);
</script>
