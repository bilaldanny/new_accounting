<script setup lang="ts">
    ...
    import type { StockAdjustmentLineRow } from '@/composables/stockadjustment';
    ...
    const adjustmentLines = computed<StockAdjustmentLineRow[]>(() => (
        Array.isArray(params.formData?.purchaselines) ? params.formData.purchaselines : []
    ));
    const itemCountLabel = computed(() => {
        const count = adjustmentLines.value.length;
    ...
    const totalIncrease = computed(() => (
        adjustmentLines.value.reduce((sum, line) => sum + Math.max(toNumber(line.quantity_adjustment), 0), 0)
    ));
    const totalDecrease = computed(() => (
        adjustmentLines.value.reduce((sum, line) => sum + Math.max(-toNumber(line.quantity_adjustment), 0), 0)
    ));
    const netChange = computed(() => (
        adjustmentLines.value.reduce((sum, line) => sum + toNumber(line.quantity_adjustment), 0)
    ));
    ...
    function persistLines(lines: StockAdjustmentLineRow[]) {
    ...
    async function fetchProductSuggestions(term = '', filters: {
        ...
        try {
            const response = await window.axios.get(API_ENDPOINTS.stockAdjustmentSearchProducts, {
                params: {
    ...
    function addProduct(product: {
        ...
        units: StockAdjustmentLineRow['units'];
    }) {
        const alreadyAdded = adjustmentLines.value.some((line) => (
        ...
        const nextLine: StockAdjustmentLineRow = {
        ...
        persistLines([nextLine, ...adjustmentLines.value]);
    }

    function updateLine(index: number, patch: Partial<StockAdjustmentLineRow>) {
        persistLines(adjustmentLines.value.map((line, lineIndex) => (
            lineIndex === index ? { ...line, ...patch } : line
        )));
    }

    function removeLine(index: number) {
        persistLines(adjustmentLines.value.filter((_, lineIndex) => lineIndex !== index));
    }
    ...
    <GroupElement name="group_details" :columns="colFull" :add-classes="cardClasses">
        <StaticElement name="section_details" :columns="colFull">
            <div class="product-form-section-head">
                ...
                <div>
                    <h2 class="product-form-section-title">Adjustment details</h2>
                    <p class="product-form-section-copy">Branch, reason type, and reference</p>
                </div>
            </div>
        </StaticElement>
        ...
        <TextElement
            id="AdjustmentRefNo"
            field-name="AdjustmentRefNo"
            name="invoice_no"
            label="Reference no"
            placeholder="Auto-generated if blank"
            :columns="colThird"
            autocomplete="off"
            info="Leave empty to generate from company stock adjustment prefix."
        />
        ...
        <SelectElement
            ...
            :floating="false"
            :can-clear="false"
            info="Completed applies the change to branch stock."
        />
    </GroupElement>
                ...
                <div class="journal-card-head__lead">
                    ...
                    <div>
                        ...
                        <p class="product-form-section-copy">
                            {{ isSearchBox
                                ? 'Search products, then set the increase or decrease amount'
        ...
        <StaticElement name="section_notes" :columns="colFull">
            <div class="product-form-section-head">
                ...
                <div>
                    ...
                    <p class="product-form-section-copy">Reason for the adjustment and a quick totals check</p>
                </div>
            </div>
        </StaticElement>

        <TextareaElement
            id="AdjustmentAdditionalNote"
            field-name="AdjustmentAdditionalNote"
            name="additional_note"
            label="Reason / note"
            placeholder="Why stock is being adjusted, count discrepancy, damage, or other remarks"
            :columns="colHalf"
            :rows="4"
        />

        <StaticElement name="adjustment_summary" :columns="colHalf">
            <div class="space-y-3.5 rounded-xl border border-slate-200/90 bg-slate-50/70 p-5">
                <div>
                    ...
                    <h3 class="text-sm font-bold text-slate-800">Adjustment totals</h3>
                </div>
                <div class="space-y-2.5 text-xs text-slate-600">
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <span class="font-mono font-semibold text-emerald-700">+{{ totalIncrease }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span>Total decrease</span>
                        <span class="font-mono font-semibold text-rose-700">-{{ totalDecrease }}</span>
                    </div>
                    <div class="flex items-center justify-between border-t border-slate-200 pt-2.5">
                        <span class="font-semibold text-slate-700">Net change</span>
                        <span class="font-mono font-bold text-slate-900">{{ netChange >= 0 ? '+' : '' }}{{ netChange }}</span>
                    </div>
                </div>
            </div>
        </StaticElement>
