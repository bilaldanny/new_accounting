<script setup lang="ts">
    ...
    defineOptions({
        layout: {
            ...
            subtitle: 'Correct branch stock levels and track adjustment reasons',
            breadcrumbs: [
                {
                    title: 'Stock Adjustment',
                    href: 'NULL',
    ...
    const columns = computed(() => [
        ...
        { key: 'adjustment_type_label', label: 'Type', type: 'secondary', responsive: ['sm', 'md', 'lg'], emptyDisplay: '-' },
    ...
    const debouncedGetStockAdjustments = debounce((params) => {
    ...
    const getData = async () => {
        try {
            ...
            await debouncedGetStockAdjustments({ ...state.search });
    ...
    onMounted(async () => {
        ...
        debouncedGetStockAdjustments({ ...state.search });
    });
    ...
    const fetchAllRowsForExport = createTableExportAllRows(API_ENDPOINTS.stockAdjustments, () => state);
        ...
        <div class="admin-list-card">
            ...
            <TheFilter v-model:open="filterOpen" :loading="state.loading" @clear="clearSearch" @search="getData">
                <div v-if="showCompanyFilter" class="col-md-4 col-lg-3 admin-filter-field">
                    <label class="form-label" for="stockadjustment-filter-company">Company</label>
                    <select
                        id="stockadjustment-filter-company"
                ...
                <div v-if="showBranchFilter" class="col-md-4 col-lg-3 admin-filter-field">
                    <label class="form-label" for="stockadjustment-filter-branch">Branch</label>
                    <select
                        id="stockadjustment-filter-branch"
                        class="form-select form-select-sm"
                ...
                <div class="col-md-4 col-lg-3 admin-filter-field">
                    <label class="form-label" for="stockadjustment-filter-type">Adjustment type</label>
                    <select id="stockadjustment-filter-type" class="form-select form-select-sm" v-model="state.search.adjustment_type">
                ...
                <div class="col-md-4 col-lg-3 admin-filter-field">
                    <label class="form-label" for="stockadjustment-filter-status">Status</label>
                    <select id="stockadjustment-filter-status" class="form-select form-select-sm" v-model="state.search.status">
                        <option value="all">All</option>
