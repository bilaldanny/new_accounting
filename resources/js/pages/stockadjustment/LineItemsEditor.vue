<script setup lang="ts">
    ...
    const props = defineProps({
        lines: {
            type: Array as () => StockAdjustmentLineRow[],
            default: () => [],
        },
        suggestions: {
    ...
    const emit = defineEmits<{
        ...
        update: [index: number, patch: Partial<StockAdjustmentLineRow>];
        remove: [index: number];
    }>();
    ...
    function direction(line: StockAdjustmentLineRow): 'increase' | 'decrease' {
        return toNumber(line.quantity_adjustment) < 0 ? 'decrease' : 'increase';
    }

    function magnitude(line: StockAdjustmentLineRow): number {
        return Math.abs(toNumber(line.quantity_adjustment));
    }

    function newStock(line: StockAdjustmentLineRow): number {
        return toNumber(line.current_stock) + toNumber(line.quantity_adjustment);
    }

    function packingLabel(line: StockAdjustmentLineRow): string {
    ...
    function changeDirection(index: number, nextDirection: string) {
        const line = props.lines[index];
        ...
        const signedMagnitude = Math.max(magnitude(line), 0);
        emit('update', index, {
            quantity_adjustment: nextDirection === 'decrease' ? -signedMagnitude : signedMagnitude,
        });
    }

    function changeMagnitude(index: number, value: string) {
        const line = props.lines[index];
        ...
        const nextMagnitude = Math.max(toNumber(value), 0);
        emit('update', index, {
            quantity_adjustment: direction(line) === 'decrease' ? -nextMagnitude : nextMagnitude,
        });
    }
</script>
...
<template>
    <div class="purchase-lines">
        ...
        <div v-if="!hasLines" class="purchase-lines__empty">
            ...
            <strong>No products added to this adjustment</strong>
            <span>{{ disabled
            ...
            <table class="pricing-table__grid">
                <thead>
                    <tr>
                        ...
                        <th class="is-direction">Direction</th>
                        <th class="is-qty">Qty / unit</th>
                        <th class="is-money">Current</th>
                        <th class="is-money">New stock</th>
                ...
                <tbody>
                    <tr v-for="(line, index) in lines" :key="line.id ?? `${line.variation_id}-${index}`">
                        ...
                        <td class="is-direction">
                            <select
                                class="purchase-lines__select-input"
                                :value="direction(line)"
                                :disabled="disabled"
                                @change="changeDirection(index, ($event.target as HTMLSelectElement).value)"
                            >
                        ...
                        <td class="is-qty">
                            <div class="purchase-lines__control is-qty">
                                <input
                                    ...
                                    :value="magnitude(line)"
                                    :disabled="disabled"
                                    @input="changeMagnitude(index, ($event.target as HTMLInputElement).value)"
                                >
                                <select
                        ...
                        <td class="is-money">
                            <span class="purchase-lines__amount">{{ line.current_stock ?? 0 }} {{ packingLabel(line) }}</span>
                        </td>
                        <td class="is-money">
                            <span
                                class="purchase-lines__amount is-strong"
                                :class="{ 'is-negative': newStock(line) < 0 }"
                            >
                                {{ newStock(line) }} {{ packingLabel(line) }}
                            </span>
                        </td>
                        <td class="is-action">
                            <button
...
.pricing-table__grid th.is-direction,
.pricing-table__grid td.is-direction {
    width: 8.5rem;
}
...
.purchase-lines__amount.is-negative {
    color: #b91c1c;
}
