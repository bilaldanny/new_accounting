<script setup lang="ts">
    ...
    defineOptions({
        layout: {
            title: 'Receiving Note Trash',
            subtitle: 'Restore or permanently delete removed receiving notes',
            breadcrumbs: [
                {
    ...
    const debouncedGetTrashReceivingNotes = debounce((params) => {
        getTrashReceivingNotes(params);
    }, 300);
    ...
    const getData = async () => {
        try {
            ...
            await debouncedGetTrashReceivingNotes({ ...state.search });
            ...
            console.error('Error fetching trash receiving notes:', error);
        }
    };
    ...
    onMounted(async () => {
        ...
        debouncedGetTrashReceivingNotes({ ...state.search });
    });
    ...
    const fetchAllRowsForExport = createTableExportAllRows(`${API_ENDPOINTS.receivingNotes}/trash`, () => state);
</script>
