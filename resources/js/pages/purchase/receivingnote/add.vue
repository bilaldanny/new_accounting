<script setup lang="ts">
    ...
    defineOptions({
        layout: {
            ...
            subtitle: 'Receive goods against an approved purchase order',
            breadcrumbs: [
                {
    ...
    const { formData, defaultFormData, emptyForm } = useReceivingNotes();
    ...
    async function submitWithLines(form$: { data?: Record<string, unknown> }) {
        const response = await window.axios.post(API_ENDPOINTS.receivingNotes, {
            ...form$?.data,
        ...
        await router.visit('/receivingnote');
...
<template>
    <Head :title="`New ${formatedText('receivingnote')}`" />
        ...
        >
            <div class="product-form product-form--sectioned purchase-form receiving-note-form">
                ...
                <TheForm
                    ...
                    :url="API_ENDPOINTS.receivingNotes"
                    ref="formRef"
                >
