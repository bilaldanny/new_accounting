<script setup lang="ts">
    ...
    defineOptions({
        layout: {
            title: 'Edit Receiving Note',
            subtitle: 'Update received quantities for this goods receipt',
            breadcrumbs: [
                {
    ...
    const { formData, getEditData } = useReceivingNotes();
    ...
    const endpoint = computed(() => `${API_ENDPOINTS.receivingNotes}/${recordId.value}`);
    ...
    async function submitWithLines(form$: { data?: Record<string, unknown> }) {
        ...
        await router.visit(saveAction.value === 'add-new' ? '/receivingnote/add' : '/receivingnote');
...
<template>
    <Head :title="`Edit ${formatedText('receivingnote')}`" />
    ...
    <div class="product-form-page purchase-form-page receiving-note-form-page">
        <PurchaseFormChrome
            ...
            @cancel="router.visit('/receivingnote')"
            @save="save"
        >
