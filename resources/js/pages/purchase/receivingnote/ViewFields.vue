...
type ReceivingNoteView = {
    ...
    purchaselines?: ReceivingNoteLine[];
};

defineProps<{
    note: ReceivingNoteView;
}>();
...
<template>
    <article class="purchase-approval-doc" id="receiving-note-print">
        <header class="purchase-approval-doc__header">
            <div>
                <p class="purchase-approval-doc__eyebrow">Goods received note</p>
                <h2 class="purchase-approval-doc__title">
                    {{ note.invoice_no ? `#${note.invoice_no}` : 'Receiving note' }}
                </h2>
                <p class="purchase-approval-doc__meta">
                    ...
                    <span v-if="note.purchase_order_no"> · PO {{ note.purchase_order_no }}</span>
                </p>
            </div>
                ...
                <table class="purchase-approval-doc__table">
                    <thead>
                        <tr>
                            <th>S#</th>
                            <th>Product name</th>
                            <th class="is-num">Purchase quantity</th>
                            <th class="is-num">Received quantity</th>
                            <th class="is-num">Remaining</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr v-if="!note.purchaselines?.length">
                            <td colspan="5" class="is-empty">No line items found</td>
                        </tr>
                        <tr v-for="(line, index) in note.purchaselines" :key="line.id ?? index">
                            <td>{{ index + 1 }}</td>
                            <td>
                            ...
                            <td class="is-num">{{ qty(line.quantity_received) }}</td>
