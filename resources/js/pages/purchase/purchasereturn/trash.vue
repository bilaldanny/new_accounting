<script setup lang="ts">
    ...
    defineOptions({
        layout: {
            title: 'Purchase Return Trash',
            subtitle: 'Restore or permanently delete removed purchase returns',
            breadcrumbs: [
                {
    ...
    const debouncedGetTrashPurchaseReturns = debounce((params) => {
        getTrashPurchaseReturns(params);
    }, 300);
    ...
    const getData = async () => {
        try {
            ...
            await debouncedGetTrashPurchaseReturns({ ...state.search });
            ...
            console.error('Error fetching trash purchase returns:', error);
        }
    };
    ...
    onMounted(async () => {
        ...
        debouncedGetTrashPurchaseReturns({ ...state.search });
    });
    ...
    const fetchAllRowsForExport = createTableExportAllRows(`${API_ENDPOINTS.purchaseReturns}/trash`, () => state);
</script>
