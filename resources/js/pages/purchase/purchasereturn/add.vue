<script setup lang="ts">
    ...
    defineOptions({
        layout: {
            ...
            subtitle: 'Return received items against a purchase order',
            breadcrumbs: [
                {
    ...
    const { formData, defaultFormData, emptyForm } = usePurchaseReturns();
    ...
    async function submitWithLines(form$: { data?: Record<string, unknown> }) {
        const response = await window.axios.post(API_ENDPOINTS.purchaseReturns, {
            ...form$?.data,
        ...
        await router.visit('/purchase/return');
...
<template>
    <Head :title="`New ${formatedText('purchase.return')}`" />
    ...
    <div class="product-form-page purchase-form-page receiving-note-form-page">
        <div class="product-form purchase-form receiving-note-form">
            ...
            <TheForm
                ...
                :url="API_ENDPOINTS.purchaseReturns"
                ref="formRef"
            >
