<script setup lang="ts">
    ...
    defineOptions({
        layout: {
            title: 'New Purchase',
            subtitle: 'Record a supplier purchase with items, costs, and totals',
            breadcrumbs: [
                {
    ...
    const { formData, defaultFormData, emptyForm } = usePurchases();
    ...
    async function submitWithLines(form$: { data?: Record<string, unknown> }) {
        const response = await window.axios.post(API_ENDPOINTS.purchases, {
            ...form$?.data,
...
<template>
    <Head :title="`New ${formatedText('purchase')}`" />
