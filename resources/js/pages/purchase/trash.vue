<script setup lang="ts">
    ...
    defineOptions({
        layout: {
            title: 'Purchase Trash',
            subtitle: 'Restore or permanently delete removed purchases',
            breadcrumbs: [
                {
    ...
    const debouncedGetTrashPurchases = debounce((params) => {
        getTrashPurchases(params);
    }, 300);
    ...
    const getData = async () => {
        try {
            ...
            await debouncedGetTrashPurchases({ ...state.search });
    ...
    onMounted(async () => {
        ...
        debouncedGetTrashPurchases({ ...state.search });
    });
    ...
    const fetchAllRowsForExport = createTableExportAllRows(`${API_ENDPOINTS.purchases}/trash`, () => state);
</script>
