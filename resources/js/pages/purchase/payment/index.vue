<script setup lang="ts">
    ...
    import usePurchasePayments from '@/composables/purchasePayment';
    ...
    defineOptions({
        layout: {
            ...
            subtitle: 'Record supplier payments against purchase orders',
            breadcrumbs: [
                {
                    title: 'Purchase Payment',
                    href: 'NULL',
    ...
    const { props } = usePage();
    const moduleUrl = 'purchase/payment';
    ...
    const columns = computed(() => [
        ...
        { key: 'invoice_no', label: 'Purchase Ref', type: 'secondary', responsive: ['xs', 'sm', 'md', 'lg'], emptyDisplay: '-' },
        { key: 'supplier_name', label: 'Supplier', type: 'secondary', responsive: ['xs', 'sm', 'md', 'lg'], emptyDisplay: '-' },
        ...
        { key: 'purchase_payment_status', label: 'Purchase Status', type: 'secondary', responsive: ['sm', 'md', 'lg'], emptyDisplay: '-' },
    ...
    const debouncedGetPurchasePayments = debounce((params) => {
    ...
    const getData = async () => {
        try {
            ...
            await debouncedGetPurchasePayments({ ...state.search });
    ...
    onMounted(async () => {
        ...
        debouncedGetPurchasePayments({ ...state.search });
    ...
    const fetchAllRowsForExport = createTableExportAllRows(API_ENDPOINTS.purchasePayments, () => state);
...
<template>
        ...
        <div class="admin-list-card">
            ...
            <TheFilter v-if="showFilter" v-model:open="filterOpen" :loading="state.loading" @clear="clearSearch" @search="getData">
                <div v-if="showCompanyFilter" class="col-md-4 col-lg-3 admin-filter-field">
                    <label class="form-label" for="purchase-payment-filter-company">Company</label>
                    <select
                        id="purchase-payment-filter-company"
                ...
                <div v-if="showBranchFilter" class="col-md-4 col-lg-3 admin-filter-field">
                    <label class="form-label" for="purchase-payment-filter-branch">Branch</label>
                    <select
                        id="purchase-payment-filter-branch"
                        class="form-select form-select-sm"
                ...
                <div class="col-md-4 col-lg-3 admin-filter-field">
                    <label class="form-label" for="purchase-payment-filter-method">Method</label>
                    <select
                        id="purchase-payment-filter-method"
                        class="form-select form-select-sm"
    ...
    <AddModal
        ...
        :formData="formData"
        :formRef="form$"
        :endpoint="API_ENDPOINTS.purchasePayments"
        :onOpen="openAddModal"
    ...
    <EditModal
        ...
        :endpoint="`${API_ENDPOINTS.purchasePayments}/${edit_id.id}`"
