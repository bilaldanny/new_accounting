<script setup lang="ts">
    ...
    const props = withDefaults(defineProps<{
        mode: 'add' | 'edit';
        entity: 'purchase' | 'sell' | 'receiving-note' | 'purchase-return' | 'issue-note';
        isBusy: boolean;
        isWorking: boolean;
    ...
    const listTitle = computed(() => {
        if (props.entity === 'sell') {
            return 'Sell Management';
        }

        if (props.entity === 'receiving-note') {
            return 'Receiving Note';
        }

        if (props.entity === 'issue-note') {
            return 'Issue Note';
        }

        if (props.entity === 'purchase-return') {
    ...
    const crumbCurrent = computed(() => {
        if (props.entity === 'receiving-note') {
            return props.mode === 'edit' ? 'Edit Receiving Note' : 'Add Receiving Note';
        }

        if (props.entity === 'issue-note') {
            return props.mode === 'edit' ? 'Edit Issue Note' : 'Add Issue Note';
        }

        if (props.entity === 'purchase-return') {
            return props.mode === 'edit' ? 'Edit Purchase Return' : 'Add Purchase Return';
        }

        if (props.mode === 'edit') {
            return props.entity === 'sell' ? 'Edit Sell' : 'Edit Purchase';
        }

        return props.entity === 'sell' ? 'Add Sell' : 'Add Purchase';
    });
    const pageTitle = computed(() => {
        ...
        if (props.entity === 'issue-note') {
            return props.mode === 'edit' ? 'Edit Issue Note' : 'New Issue Note';
        }

        if (props.entity === 'purchase-return') {
            return props.mode === 'edit' ? 'Edit Purchase Return' : 'New Purchase Return';
        }
        ...
        return props.entity === 'sell' ? 'Add New Sell' : 'Add New Purchase';
    });
</script>
                ...
                <div class="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
                    <div>
                        ...
                        <h1 class="flex items-center gap-2 text-lg font-bold tracking-tight text-slate-900 sm:text-xl">
                            ...
                            <span
                                v-if="referenceNo"
                                class="rounded-full border border-slate-200 bg-slate-100 px-2 py-0.5 font-mono text-xs font-semibold text-slate-700"
                            >
                                {{ referenceNo }}
                            </span>
                        </h1>
