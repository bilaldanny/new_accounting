<script setup lang="ts">
    ...
    import { state as stateRoute } from '@/routes';
    ...
    defineOptions({
        layout: {
            title: 'State Trash',
            subtitle: 'Restore or permanently delete removed states',
            breadcrumbs: [
                {
    ...
    const debouncedGetTrashStates = debounce((params) => {
        getTrashStates(params);
    }, 300);
    ...
    const getData = async () => {
        try {
            ...
            await debouncedGetTrashStates({ ...state.search });
    ...
    onMounted(() => {
        ...
        debouncedGetTrashStates({ ...state.search });
    });

    const fetchAllRowsForExport = createTableExportAllRows(`${API_ENDPOINTS.states}/trash`, () => state);
</script>
