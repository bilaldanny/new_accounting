        ...
        <div v-else class="supplier-view-layout">
            <aside class="supplier-view-sidebar">
                ...
                <section v-else class="admin-list-card supplier-view-profile">
                    ...
                    <div class="supplier-view-profile__body">
                    ...
                    <div class="supplier-view-profile__details">
                        <button
                            type="button"
                            class="supplier-view-profile__details-toggle"
                            :aria-expanded="detailsOpen"
                            @click="detailsOpen = !detailsOpen"
                        >
                            <h2 class="supplier-view-profile__details-title">Contact details</h2>
                            <ChevronDown size="sm" :class="{ 'supplier-view-scope__chevron--open': detailsOpen }" />
                        </button>

                        <ul v-show="detailsOpen" class="supplier-view-info-list">
                ...
                <section class="admin-list-card supplier-view-workspace">
                    ...
                    <div class="supplier-view-tabs__panel">
                        <div v-if="activeTab === 'ledger'" class="supplier-view-ledger">
                            ...
                            <template v-else>
                                <div class="supplier-view-ledger-kpis">
                                    <article class="supplier-view-ledger-kpi">
                                        <span>Opening balance</span>
                                        <strong>{{ currencyCode }} {{ formatAmount(ledgerData.openingbalance) }}</strong>
                                    </article>
                                    <article v-if="showPurchaseStats" class="supplier-view-ledger-kpi">
                                        <span>Total purchase</span>
                                        <strong>{{ currencyCode }} {{ formatAmount(ledgerData.total_purchase) }}</strong>
                                    </article>
                                    <article v-if="showPurchaseStats" class="supplier-view-ledger-kpi">
                                        <span>Purchase paid</span>
                                        <strong>{{ currencyCode }} {{ formatAmount(ledgerData.total_paid_purchase) }}</strong>
                                    </article>
                                    <article v-if="showSellStats" class="supplier-view-ledger-kpi">
                                    ...
                                    <article class="supplier-view-ledger-kpi supplier-view-ledger-kpi--accent">
                                ...
                                <div class="supplier-view-table-wrap">
                                    <table class="table supplier-view-table">
                                        <thead>
                                            <tr>
                                                <th class="supplier-view-table__check" title="Reviewed up to this transaction">
                                        ...
                                        <tbody>
                                            <tr class="supplier-view-table__opening">
                                                <td class="supplier-view-table__check"></td>
                                            ...
                                            <tr
                                                ...
                                                :class="{
                                                    'supplier-view-table__highlight': row.highlight === 1,
                                                    'supplier-view-table__watched': isRowWatched(ledgerRows, row),
                                                }"
                                            >
                                                <td class="supplier-view-table__check">
                                                    <input
                                                        type="checkbox"
                                        ...
                                        <tfoot v-if="ledgerRows.length > 0">
                                            <tr class="supplier-view-table__footer">
                                                <td colspan="7">Period totals</td>
                                                <td class="text-end supplier-view-amount">{{ formatAmount(ledgerDebitTotal) }}</td>
                                                <td class="text-end supplier-view-amount">{{ formatAmount(ledgerCreditTotal) }}</td>
                                                <td class="text-end supplier-view-amount supplier-view-amount--strong">
...
.supplier-view-profile__details-toggle {
    display: flex;
    align-items: center;
    justify-content: space-between;
    width: 100%;
...
.supplier-view-profile__details-toggle svg {
    color: var(--app-text-secondary, #64748b);
...
.supplier-view-ledger-kpi strong {
    font-size: 1.0625rem;
    font-weight: 800;
    color: var(--app-text, #111827);
    font-variant-numeric: tabular-nums;
...
.supplier-view-table__check {
    width: 2.5rem;
    text-align: center;
}

.supplier-view-table__check input {
    width: 1rem;
    height: 1rem;
    margin: 0;
...
.supplier-view-table__watched:not(.supplier-view-table__highlight) {
    --bs-table-bg: #c6efe8;
    --bs-table-accent-bg: #c6efe8;
}

.supplier-view-table__watched:not(.supplier-view-table__highlight) > td {
    background-color: #c6efe8 !important;
    color: #0f766e;
    font-weight: 600;
}

.supplier-view-table__watched:not(.supplier-view-table__highlight) > td:first-child {
    box-shadow: inset 4px 0 0 var(--sv-accent);
}

.supplier-view-table__row.supplier-view-table__watched:hover > td {
...
.supplier-view-table__footer {
    background: #f8fafc;
    font-weight: 700;
}

.supplier-view-table__footer td {
    border-top: 1px solid #e2e8f0;
    color: var(--app-text, #111827);
}
