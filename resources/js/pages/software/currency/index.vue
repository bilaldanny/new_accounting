<script setup lang="ts">
    ...
    const columns = [
        ...
        { key: 'currency_name', label: 'Name', type: 'primary', responsive: ['sm', 'md', 'lg'] },
        { key: 'code', label: 'Code', type: 'code', responsive: ['md', 'lg'] },
        { key: 'symbol', label: 'Symbol', type: 'secondary', responsive: ['lg'] },
    ...
    const debouncedGetCurrencies = debounce((params) => {
    ...
    const getData = async () => {
        try {
            ...
            await debouncedGetCurrencies({ ...state.search });
    ...
    onMounted(() => {
        ...
        debouncedGetCurrencies({ ...state.search });
    });
    ...
    const fetchAllRowsForExport = createTableExportAllRows(API_ENDPOINTS.currencies, () => state);
...
<template>
        ...
        <div class="admin-list-card">
            ...
            <TheFilter v-model:open="filterOpen" :loading="state.loading" @clear="clearSearch" @search="getData">
                <div class="col-md-4 col-lg-3 admin-filter-field">
                    <label class="form-label" for="currency-filter-status">Status</label>
                    <select
                        id="currency-filter-status"
                ...
                <div class="col-md-4 col-lg-3 admin-filter-field">
                    <label class="form-label" for="currency-filter-records">Show records</label>
                    <select
                        id="currency-filter-records"
                        class="form-select form-select-sm"
            ...
            <div class="admin-list-card__body">
                <Loader v-if="state.fetchingApi" message="Fetching currencies from API…" :fields="6" />
    ...
    <EditModal
        ...
        :endpoint="`${API_ENDPOINTS.currencies}/${edit_id.id}`"
