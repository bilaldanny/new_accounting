<script setup lang="ts">
    ...
    defineOptions({
        layout: {
            ...
            subtitle: 'Manage states and provinces used across the application',
            breadcrumbs: [
                {
                    title: 'State Management',
                    href: 'NULL',
    ...
    const { select_data, getSavedValue, formatedText, handleError, handleSuccess, fetchCountry, countriesdata } = useCommons();
    ...
    const columns = [
        ...
        { key: 'iso2', label: 'ISO2', type: 'code', responsive: ['lg'] },
    ...
    const debouncedGetStates = debounce((params) => {
        getStates(params);
    }, 300);
    ...
    const getData = async () => {
        try {
            ...
            await debouncedGetStates({ ...state.search });
    ...
    onMounted(async () => {
        ...
        debouncedGetStates({ ...state.search });
    });
    ...
    const fetchAllRowsForExport = createTableExportAllRows(API_ENDPOINTS.states, () => state);
...
<template>
        ...
        <div class="admin-list-card">
            ...
            <TheFilter v-model:open="filterOpen" :loading="state.loading" @clear="clearSearch" @search="getData">
                <div class="col-md-4 col-lg-3 admin-filter-field">
                    <label class="form-label" for="state-filter-country">Country</label>
                    <select
                ...
                <div class="col-md-4 col-lg-3 admin-filter-field">
                    <label class="form-label" for="state-filter-status">Status</label>
                    <select
                ...
                <div class="col-md-4 col-lg-3 admin-filter-field">
                    <label class="form-label" for="state-filter-records">Show records</label>
                    <select
                        id="state-filter-records"
                        class="form-select form-select-sm"
            ...
            <div class="admin-list-card__body">
                <Loader v-if="state.fetchingApi" message="Fetching states for the next country…" :fields="6" />
    ...
    <EditModal
        ...
        :endpoint="`${API_ENDPOINTS.states}/${edit_id.id}`"
