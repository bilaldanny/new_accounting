<script setup lang="ts">
    ...
    defineOptions({
        layout: {
            title: 'Country Trash',
            subtitle: 'Restore or permanently delete removed countries',
            breadcrumbs: [
                {
    ...
    const debouncedGetTrashCountries = debounce((params) => {
        getTrashCountries(params);
    }, 300);
    ...
    const getData = async () => {
        try {
            ...
            await debouncedGetTrashCountries({ ...state.search });
    ...
    onMounted(() => {
        ...
        debouncedGetTrashCountries({ ...state.search });
    });

    const fetchAllRowsForExport = createTableExportAllRows(`${API_ENDPOINTS.countries}/trash`, () => state);
</script>
