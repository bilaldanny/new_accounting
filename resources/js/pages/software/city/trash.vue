<script setup lang="ts">
    ...
    defineOptions({
        layout: {
            title: 'City Trash',
            subtitle: 'Restore or permanently delete removed cities',
            breadcrumbs: [
                {
    ...
    const debouncedGetTrashCities = debounce((params) => {
        getTrashCities(params);
    }, 300);
    ...
    const getData = async () => {
        try {
            ...
            await debouncedGetTrashCities({ ...state.search });
    ...
    onMounted(() => {
        ...
        debouncedGetTrashCities({ ...state.search });
    });

    const fetchAllRowsForExport = createTableExportAllRows(`${API_ENDPOINTS.cities}/trash`, () => state);
</script>
