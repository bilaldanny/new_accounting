<script setup lang="ts">
    ...
    import type { ExpenseLineRow } from '@/composables/expense';
    ...
    const emit = defineEmits<{
        add: [line: ExpenseLineRow];
        update: [index: number, line: ExpenseLineRow];
        remove: [index: number];
    }>();

    const draft = ref<ExpenseLineRow>({
        account_id: '',
        description: '',
        debit: '',
        credit: '',
    ...
    function applyAccount(line: ExpenseLineRow, accountId: number | string | ''): ExpenseLineRow {
    ...
    function duplicateLine(line: ExpenseLineRow) {
        if (props.disabled) {
            return;
        }
                ...
                <tbody>
                    <tr v-if="lines.length === 0 && editingIndex === null">
                        <td colspan="6" class="journal-lines__empty">
                            ...
                            <small>Select the expense account below, enter a debit or credit amount, then add the line to balance this voucher.</small>
                        </td>
                    </tr>
                    <tr
