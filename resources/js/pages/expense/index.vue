<script setup lang="ts">
    ...
    defineOptions({
        layout: {
            title: 'Expense',
            subtitle: 'Bills, receipts, and other business expenses',
            breadcrumbs: [
                {
                    title: 'Expense',
                    href: 'NULL',
    ...
    const debouncedGetExpenses = debounce((params) => {
        getExpenses(params);
    }, 300);
    ...
    const getData = async () => {
        try {
            ...
            await debouncedGetExpenses({ ...state.search });
    ...
    onMounted(async () => {
        ...
        debouncedGetExpenses({ ...state.search });
    });
    ...
    const fetchAllRowsForExport = createTableExportAllRows(API_ENDPOINTS.expenses, () => state);
        ...
        <div class="admin-list-card">
            ...
            <TheFilter v-if="showFilter" v-model:open="filterOpen" :loading="state.loading" @clear="clearSearch" @search="getData">
                <div v-if="showCompanyFilter" class="col-md-4 col-lg-3 admin-filter-field">
                    <label class="form-label" for="expense-filter-company">Company</label>
                    <select
                        id="expense-filter-company"
                ...
                <div v-if="showBranchFilter" class="col-md-4 col-lg-3 admin-filter-field">
                    <label class="form-label" for="expense-filter-branch">Branch</label>
                    <select
                        id="expense-filter-branch"
                        class="form-select form-select-sm"
                ...
                <div class="col-md-4 col-lg-3 admin-filter-field">
                    <label class="form-label" for="expense-filter-status">Status</label>
                    <select id="expense-filter-status" class="form-select form-select-sm" v-model="state.search.status">
