<script setup lang="ts">
    ...
    import useConsumers from '@/composables/consumer';
    ...
    defineOptions({
        layout: {
            title: 'Consumer Trash',
            subtitle: 'Restore or permanently delete removed consumers',
            breadcrumbs: [
                {
                    title: 'Consumer',
                    href: '/consumer',
    ...
    const columns = [
        ...
        { key: 'name', label: 'Consumer', type: 'primary', responsive: ['xs', 'sm', 'md', 'lg'] },
        { key: 'branch_name', label: 'Branch', type: 'secondary', responsive: ['xs', 'sm', 'md', 'lg'], emptyDisplay: '-' },
        { key: 'phone_res', label: 'Phone', type: 'secondary', responsive: ['sm', 'md', 'lg'], emptyDisplay: '-' },
    ...
    const debouncedGetTrashConsumers = debounce((params) => {
        getTrashConsumers(params);
    }, 300);
    ...
    const getData = async () => {
        try {
            ...
            await debouncedGetTrashConsumers({ ...state.search });
            ...
            console.error('Error fetching trash consumers:', error);
        }
    };
    ...
    onMounted(async () => {
        ...
        debouncedGetTrashConsumers({ ...state.search });
    });
    ...
    const fetchAllRowsForExport = createTableExportAllRows(`${API_ENDPOINTS.consumers}/trash`, () => state);
</script>
