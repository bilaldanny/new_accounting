<script setup lang="ts">
    ...
    import ConsumerFormChrome from './ConsumerFormChrome.vue';
    ...
    defineOptions({
        layout: {
            title: 'Edit Consumer',
            subtitle: 'Update consumer details and location',
            breadcrumbs: [
                {
                    title: 'Consumer',
                    href: '/consumer',
    ...
    const { formData, getEditData } = useConsumers();
    ...
    const endpoint = computed(() => `${API_ENDPOINTS.consumers}/${recordId.value}`);

    function handleFormError(error: unknown, details?: unknown) {
        handleError(error, details, formRef);
    }

    async function submitConsumer(form$: { data?: Record<string, unknown> }) {
        ...
        await router.visit(saveAction.value === 'add-new' ? '/consumer/add' : '/consumer');
...
<template>
    <Head :title="`Edit ${formatedText('consumer')}`" />
    ...
    <div class="product-form-page purchase-form-page">
        <ConsumerFormChrome
            ...
            @cancel="router.visit('/consumer')"
            @save="save"
        >
            <div class="product-form product-form--sectioned purchase-form">
                <Loader v-if="!pageReady" message="Loading consumer…" />
