<script setup lang="ts">
    ...
    defineOptions({
        layout: {
            title: 'New Consumer',
            subtitle: 'Add a consumer / buyer record for a branch',
            breadcrumbs: [
                {
                    title: 'Consumer',
                    href: '/consumer',
    ...
    const { formData, defaultFormData, emptyForm } = useConsumers();
    ...
    async function submitConsumer(form$: { data?: Record<string, unknown> }) {
        const response = await window.axios.post(API_ENDPOINTS.consumers, {
            ...form$?.data,
        });
...
<template>
    <Head :title="`New ${formatedText('consumer')}`" />
