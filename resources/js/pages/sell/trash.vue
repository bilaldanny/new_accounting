<script setup lang="ts">
    ...
    defineOptions({
        layout: {
            title: 'Sell Trash',
            subtitle: 'Restore or permanently delete removed sells',
            breadcrumbs: [
                {
    ...
    const columns = [
        ...
        { key: 'invoice_no', label: 'Invoice No', type: 'primary', responsive: ['sm', 'md', 'lg'] },
    ...
    const debouncedGetTrashSells = debounce((params) => {
        getTrashSells(params);
    }, 300);
    ...
    const getData = async () => {
        try {
            ...
            await debouncedGetTrashSells({ ...state.search });
            ...
            console.error('Error fetching trash sells:', error);
        }
    };
    ...
    onMounted(async () => {
        ...
        debouncedGetTrashSells({ ...state.search });
    });
    ...
    const fetchAllRowsForExport = createTableExportAllRows(`${API_ENDPOINTS.sells}/trash`, () => state);
</script>
