<script setup lang="ts">
    ...
    defineOptions({
        layout: {
            title: 'Issue Note Trash',
            subtitle: 'Restore or permanently delete removed issue notes',
            breadcrumbs: [
                {
                    title: 'Issue Note',
                    href: '/issuenote',
    ...
    const debouncedgetTrashIssueNotes = debounce((params) => {
        getTrashIssueNotes(params);
    }, 300);
    ...
    const getData = async () => {
        try {
            ...
            await debouncedgetTrashIssueNotes({ ...state.search });
            ...
            console.error('Error fetching trash issue notes:', error);
        }
    };
    ...
    onMounted(async () => {
        ...
        debouncedgetTrashIssueNotes({ ...state.search });
    });
    ...
    const fetchAllRowsForExport = createTableExportAllRows(`${API_ENDPOINTS.issueNotes}/trash`, () => state);
</script>
