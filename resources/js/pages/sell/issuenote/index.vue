<script setup lang="ts">
    ...
    defineOptions({
        layout: {
            title: 'Issue Note',
            subtitle: 'Issue goods against approved sell invoices',
            breadcrumbs: [
                {
                    title: 'Issue Note',
                    href: 'NULL',
    ...
    const columns = computed(() => [
        ...
        { key: 'sell_order_no', label: 'Sell Invoice No', type: 'secondary', responsive: ['sm', 'md', 'lg'], emptyDisplay: '-' },
    ...
    const debouncedgetIssueNotes = debounce((params) => {
        getIssueNotes(params);
    }, 300);
    ...
    const getData = async () => {
        try {
            ...
            await debouncedgetIssueNotes({ ...state.search });
    ...
    onMounted(async () => {
        ...
        debouncedgetIssueNotes({ ...state.search });
    });
    ...
    const fetchAllRowsForExport = createTableExportAllRows(API_ENDPOINTS.issueNotes, () => state);
        ...
        <div class="admin-list-card">
            ...
            <TheFilter v-if="showFilter" v-model:open="filterOpen" :loading="state.loading" @clear="clearSearch" @search="getData">
                <div v-if="showCompanyFilter" class="col-md-4 col-lg-3 admin-filter-field">
                    <label class="form-label" for="issue-note-filter-company">Company</label>
                    <select
                        id="issue-note-filter-company"
                ...
                <div v-if="showBranchFilter" class="col-md-4 col-lg-3 admin-filter-field">
                    <label class="form-label" for="issue-note-filter-branch">Branch</label>
                    <select
                        id="issue-note-filter-branch"
                        class="form-select form-select-sm"
                ...
                <div class="col-md-4 col-lg-3 admin-filter-field">
                    <label class="form-label" for="issue-note-filter-customer">Customer</label>
                    <select
                        id="issue-note-filter-customer"
                        class="form-select form-select-sm"
                ...
                <div class="col-md-4 col-lg-3 admin-filter-field">
                    <label class="form-label" for="issue-note-filter-payment">Payment status</label>
                    <select id="issue-note-filter-payment" class="form-select form-select-sm" v-model="state.search.payment_status">
                ...
                <div class="col-md-4 col-lg-3 admin-filter-field">
                    <label class="form-label" for="issue-note-filter-date">Sell date</label>
                    <input
                        id="issue-note-filter-date"
                        type="date"
            ...
            <div class="admin-list-card__body">
                <div class="admin-list-table">
                    <TheTable
                        ...
                        :delete="deleteRecord"
                        :view-route="(id) => `/issuenote/${id}/view`"
                        actionType="link"
