<script setup lang="ts">
    ...
    defineOptions({
        layout: {
            title: 'New Issue Note',
            subtitle: 'Issue goods against an approved sell invoice',
            breadcrumbs: [
                {
                    title: 'Issue Note',
                    href: '/issuenote',
    ...
    const { formData, defaultFormData, emptyForm } = useIssueNotes();
    ...
    async function submitWithLines(form$: { data?: Record<string, unknown> }) {
        const response = await window.axios.post(API_ENDPOINTS.issueNotes, {
            ...form$?.data,
...
<template>
    <Head :title="`New ${formatedText('issuenote')}`" />
