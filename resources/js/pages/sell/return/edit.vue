<script setup lang="ts">
    ...
    defineOptions({
        layout: {
            ...
            subtitle: 'Update returned quantities for this sell return',
            breadcrumbs: [
                {
                    title: 'Sell Return',
                    href: '/sell/return',
    ...
    const { formData, getEditData } = useSellReturns();
    ...
    const endpoint = computed(() => `${API_ENDPOINTS.sellReturns}/${recordId.value}`);
    ...
    async function submitWithLines(form$: { data?: Record<string, unknown> }) {
        ...
        await router.visit(saveAction.value === 'add-new' ? '/sell/return/add' : '/sell/return');
...
<template>
    <Head :title="`Edit ${formatedText('sell.return')}`" />
