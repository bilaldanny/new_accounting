<script setup lang="ts">
    ...
    defineOptions({
        layout: {
            title: 'Sell Return Trash',
            subtitle: 'Restore or permanently delete removed sell returns',
            breadcrumbs: [
                {
                    title: 'Sell Return',
                    href: '/sell/return',
    ...
    const columns = [
        ...
        { key: 'invoice_no', label: 'Ref No', type: 'primary', responsive: ['sm', 'md', 'lg'] },
    ...
    const debouncedGetTrashSellReturns = debounce((params) => {
        getTrashSellReturns(params);
    }, 300);
    ...
    const getData = async () => {
        try {
            ...
            await debouncedGetTrashSellReturns({ ...state.search });
            ...
            console.error('Error fetching trash sell returns:', error);
        }
    };
    ...
    onMounted(async () => {
        ...
        debouncedGetTrashSellReturns({ ...state.search });
    });
    ...
    const fetchAllRowsForExport = createTableExportAllRows(`${API_ENDPOINTS.sellReturns}/trash`, () => state);
</script>
        ...
        <div class="admin-list-card">
            <div class="admin-list-card__toolbar">
                <TopButtons
                    :state="state"
                    :getData="getData"
                    :restore-bulk="restoreBulkRecord"
                    :permanent-delete-bulk="perDeleteBulkRecord"
                    :url="moduleUrl"
                    type="trash"
                />
            </div>
