<script setup lang="ts">
    ...
    const emit = defineEmits<{
        ...
        update: [index: number, patch: Partial<LabelProductRow>];
        remove: [index: number];
    }>();
    ...
    const totalLabels = computed(() => props.products.reduce((sum, item) => sum + Number(item.label || 0), 0));
    ...
    <div class="purchase-lines">
        <div class="purchase-lines__search" :class="{ 'is-disabled': disabled, 'is-open': open && !disabled }">
            ...
            <input
                ...
                :placeholder="disabled ? 'Select company and branch first' : 'Search name or SKU to add a product'"
                :value="query"
                :disabled="disabled"
                autocomplete="off"
                @input="onSearch"
        ...
        <div v-if="!hasProducts" class="purchase-lines__empty">
            <Package size="md" class="purchase-lines__empty-icon" />
            <strong>No products selected</strong>
            <span>{{ disabled ? 'Choose a company and branch first.' : 'Search by name or SKU to add products to print.' }}</span>
        </div>
            ...
            <table class="pricing-table__grid">
                <thead>
                    <tr>
                        ...
                        <th class="is-labels">Labels</th>
                        <th class="is-action"></th>
                    </tr>
                </thead>
                <tbody>
                    <tr v-for="(item, index) in products" :key="item.id ?? `${item.product_id}-${index}`">
                        <td class="is-count">{{ index + 1 }}</td>
                        <td class="is-name">
                            <div class="purchase-lines__product">
                                <span class="pricing-table__name" :title="item.pro_name">{{ item.pro_name }}</span>
                                <span v-if="item.sku" class="purchase-lines__sku">{{ item.sku }}</span>
                            </div>
                        </td>
                        <td class="is-labels">
                            <input
                ...
                <tfoot>
                    <tr class="purchase-lines__totals-row">
                        <td colspan="2" class="purchase-lines__totals-label">Total labels to print:</td>
                        <td colspan="2" class="purchase-lines__totals-value">{{ totalLabels }}</td>
                    </tr>
                </tfoot>
            </table>
        </div>
...
.pricing-table__grid th.is-labels,
.pricing-table__grid td.is-labels {
    width: 7rem;
}
...
.purchase-lines__totals-row {
    background: #f8fafc;
}

.purchase-lines__totals-label {
    padding: 0.6rem;
    font-size: 0.75rem;
    font-weight: 700;
    color: #475569;
