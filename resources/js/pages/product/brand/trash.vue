<script setup lang="ts">
    ...
    defineOptions({
        layout: {
            title: 'Brand Trash',
            subtitle: 'Restore or permanently delete removed brands',
            breadcrumbs: [
                {
    ...
    const debouncedGetTrashBrands = debounce((params) => {
        getTrashBrands(params);
    }, 300);
    ...
    const getData = async () => {
        try {
            ...
            await debouncedGetTrashBrands({ ...state.search });
            ...
            console.error('Error fetching trash brands:', error);
        }
    };
    ...
    onMounted(async () => {
        ...
        debouncedGetTrashBrands({ ...state.search });
    });
    ...
    const fetchAllRowsForExport = createTableExportAllRows(`${API_ENDPOINTS.brands}/trash`, () => state);
