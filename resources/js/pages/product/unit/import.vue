<script setup lang="ts">
    ...
    const modalProps = defineProps({
        ...
        sampleFileName: { type: String, default: 'unit-import-sample.xlsx' },
    });
    ...
    const sampleColumns = ['id', 'name', 'short_name', 'type', 'parent', 'active', 'auto_adjustment'] as const;
    const fallbackSampleRows = [['', 'Kilogram', 'KG', 'large', '', 1, 0]];

    function mapUnitToSampleRow(unit: Record<string, unknown>): (string | number)[] {
        return [
            normalizeImportExportValue(unit.id),
            normalizeImportExportValue(unit.name),
            normalizeImportExportValue(unit.short_name),
            normalizeImportExportValue(unit.type ?? 'large'),
            normalizeImportExportValue(unit.parent_name ?? unit.parent_id),
            normalizeImportBool(unit.active),
            normalizeImportBool(unit.auto_adjustment),
        ];
    }
    ...
    async function downloadSampleFile() {
        ...
        try {
            await downloadImportTemplate({
                ...
                listApiUrl: API_ENDPOINTS.units,
                listQuery: { sort_type: 'desc' },
                mapRow: mapUnitToSampleRow,
    ...
    async function submitImport() {
        ...
        try {
            ...
            if (rows.length === 0) {
                fileError.value = 'The selected file does not contain any unit rows.';

                return;
            }

            const response = await window.axios.post(API_ENDPOINTS.unitImport, { rows });
...
<template>
    <ImportModalLayout
        ...
        file-input-id="unit-import-file"
        :selected-file="selectedFile"
        :file-error="fileError"
        :is-downloading-sample="isDownloadingSample"
        :is-importing="isImporting"
        :import-result="importResult"
        lead-text="Name and short name are required on every row."
        secondary-text="The template includes existing records for updates. Leave id blank to add new rows. Parent accepts a unit name or id."
