<script setup lang="ts">
    ...
    defineOptions({
        layout: {
            ...
            subtitle: 'Manage product units and conversions',
            breadcrumbs: [
                {
                    title: 'Unit Management',
                    href: 'NULL',
    ...
    const columns = computed(() => [
        ...
        { key: 'parent_name', label: 'Parent', type: 'secondary', responsive: ['xs', 'sm', 'md', 'lg'], emptyDisplay: '-' },
        { key: 'short_name', label: 'Short Name', type: 'secondary', responsive: ['xs', 'sm', 'md', 'lg'], emptyDisplay: '-' },
    ...
    const debouncedGetUnits = debounce((params) => {
        getUnits(params);
    }, 300);
    ...
    const getData = async () => {
        try {
            ...
            await debouncedGetUnits({ ...state.search });
    ...
    onMounted(async () => {
        ...
        debouncedGetUnits({ ...state.search });
    });
    ...
    const fetchAllRowsForExport = createTableExportAllRows(API_ENDPOINTS.units, () => state);
...
<template>
        ...
        <div class="admin-list-card">
            ...
            <TheFilter v-if="showFilter" v-model:open="filterOpen" :loading="state.loading" @clear="clearSearch" @search="getData">
                <div v-if="showCompanyFilter" class="col-md-4 col-lg-3 admin-filter-field">
                    <label class="form-label" for="unit-filter-company">Company</label>
                    <select
                        id="unit-filter-company"
    ...
    <EditModal
        ...
        :endpoint="`${API_ENDPOINTS.units}/${edit_id.id}`"
