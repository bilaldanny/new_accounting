<script setup lang="ts">
    ...
    defineOptions({
        layout: {
            ...
            subtitle: 'Classify products by type for inventory and reporting',
            breadcrumbs: [
                {
                    title: 'Item Type Management',
                    href: 'NULL',
    ...
    const debouncedGetItemTypes = debounce((params) => {
        getItemTypes(params);
    }, 300);
    ...
    const getData = async () => {
        try {
            ...
            await debouncedGetItemTypes({ ...state.search });
    ...
    onMounted(async () => {
        ...
        debouncedGetItemTypes({ ...state.search });
    });
    ...
    const fetchAllRowsForExport = createTableExportAllRows(API_ENDPOINTS.itemTypes, () => state);
...
<template>
        ...
        <div class="admin-list-card">
            ...
            <TheFilter v-if="showFilter" v-model:open="filterOpen" :loading="state.loading" @clear="clearSearch" @search="getData">
                <div v-if="showCompanyFilter" class="col-md-4 col-lg-3 admin-filter-field">
                    <label class="form-label" for="itemtype-filter-company">Company</label>
                    <select
                        id="itemtype-filter-company"
    ...
    <EditModal
        ...
        :endpoint="`${API_ENDPOINTS.itemTypes}/${edit_id.id}`"
