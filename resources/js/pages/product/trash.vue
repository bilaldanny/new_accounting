<script setup lang="ts">
    ...
    defineOptions({
        layout: {
            title: 'Product Trash',
            subtitle: 'Restore or permanently delete removed products',
            breadcrumbs: [
                {
    ...
    const debouncedGetTrashProducts = debounce((params) => {
        getTrashProducts(params);
    }, 300);
    ...
    const getData = async () => {
        try {
            ...
            await debouncedGetTrashProducts({ ...state.search });
    ...
    onMounted(async () => {
        ...
        debouncedGetTrashProducts({ ...state.search });
    });
    ...
    const fetchAllRowsForExport = createTableExportAllRows(`${API_ENDPOINTS.products}/trash`, () => state);
</script>
