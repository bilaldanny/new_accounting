<script setup lang="ts">
    ...
    const modalProps = defineProps({
        ...
        sampleFileName: { type: String, default: 'warranty-import-sample.xlsx' },
    });
    ...
    const sampleColumns = ['id', 'name', 'duration', 'type', 'active'] as const;
    const fallbackSampleRows = [['', 'Standard Warranty', 12, 'month', 1]];

    function mapWarrantyToSampleRow(warranty: Record<string, unknown>): (string | number)[] {
        return [
            normalizeImportExportValue(warranty.id),
            normalizeImportExportValue(warranty.name),
            normalizeImportExportValue(warranty.duration),
            normalizeImportExportValue(warranty.type ?? 'day'),
            normalizeImportBool(warranty.active),
        ];
    }
    ...
    async function downloadSampleFile() {
        ...
        try {
            await downloadImportTemplate({
                ...
                listApiUrl: API_ENDPOINTS.warranties,
                listQuery: { sort_type: 'desc' },
                mapRow: mapWarrantyToSampleRow,
    ...
    async function submitImport() {
        ...
        try {
            ...
            if (rows.length === 0) {
                fileError.value = 'The selected file does not contain any warranty rows.';

                return;
            }

            const response = await window.axios.post(API_ENDPOINTS.warrantyImport, { rows });
...
<template>
    <ImportModalLayout
        ...
        file-input-id="warranty-import-file"
        :selected-file="selectedFile"
        :file-error="fileError"
        :is-downloading-sample="isDownloadingSample"
        :is-importing="isImporting"
        :import-result="importResult"
        lead-text="Name, duration, and type are required on every row."
        secondary-text="The template includes existing records for updates. Leave id blank to add new rows. Type accepts day, month, or year."
