<script setup lang="ts">
    ...
    defineOptions({
        layout: {
            title: 'Variation Trash',
            subtitle: 'Restore or permanently delete removed variation sets',
            breadcrumbs: [
                {
    ...
    const columns = [
        ...
        { key: 'category_name', label: 'Category', type: 'secondary', responsive: ['sm', 'md', 'lg'], emptyDisplay: '-' },
        { key: 'itemtype_name', label: 'Item Type', type: 'secondary', responsive: ['sm', 'md', 'lg'], emptyDisplay: '-' },
        { key: 'values_display', label: 'Values', type: 'secondary', responsive: ['sm', 'md', 'lg'], emptyDisplay: '-' },
    ...
    const debouncedGetTrashVariations = debounce((params) => {
        getTrashVariations(params);
    }, 300);
    ...
    const getData = async () => {
        try {
            ...
            await debouncedGetTrashVariations({ ...state.search });
            ...
            console.error('Error fetching trash variations:', error);
        }
    };
    ...
    onMounted(async () => {
        ...
        debouncedGetTrashVariations({ ...state.search });
    });
    ...
    const fetchAllRowsForExport = createTableExportAllRows(`${API_ENDPOINTS.variations}/trash`, () => state);
