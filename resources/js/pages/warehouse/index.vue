<script setup lang="ts">
    ...
    import useWarehouses from '@/composables/warehouse';
    import debounce from '@/utils/debounce';

    defineOptions({
        layout: {
            title: 'Warehouse',
            subtitle: 'Manage warehouse locations for each branch',
            breadcrumbs: [
                {
                    title: 'Warehouse',
                    href: 'NULL',
    ...
    const columns = computed(() => [
        ...
        { key: 'name', label: 'Warehouse', type: 'primary', responsive: ['xs', 'sm', 'md', 'lg'] },
        { key: 'city_name', label: 'City', type: 'secondary', responsive: ['sm', 'md', 'lg'], emptyDisplay: '-' },
    ...
    const debouncedGetWarehouses = debounce((params) => {
    ...
    const getData = async () => {
        try {
            ...
            await debouncedGetWarehouses({ ...state.search });
    ...
    onMounted(async () => {
        ...
        debouncedGetWarehouses({ ...state.search });
    });
    ...
    const fetchAllRowsForExport = createTableExportAllRows(API_ENDPOINTS.warehouses, () => state);
        ...
        <div class="admin-list-card">
            ...
            <TheFilter v-model:open="filterOpen" :loading="state.loading" @clear="clearSearch" @search="getData">
                <div v-if="showCompanyFilter" class="col-md-4 col-lg-3 admin-filter-field">
                    <label class="form-label" for="warehouse-filter-company">Company</label>
                    <select
                        id="warehouse-filter-company"
                ...
                <div v-if="showBranchFilter" class="col-md-4 col-lg-3 admin-filter-field">
                    <label class="form-label" for="warehouse-filter-branch">Branch</label>
                    <select
                        id="warehouse-filter-branch"
                        class="form-select form-select-sm"
