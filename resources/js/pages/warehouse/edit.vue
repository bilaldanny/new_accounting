<script setup lang="ts">
    ...
    import WarehouseFormChrome from './WarehouseFormChrome.vue';
    ...
    defineOptions({
        layout: {
            ...
            subtitle: 'Update warehouse details and location',
            breadcrumbs: [
                {
                    title: 'Warehouse',
                    href: '/warehouse',
    ...
    const { formData, getEditData } = useWarehouses();
    ...
    const endpoint = computed(() => `${API_ENDPOINTS.warehouses}/${recordId.value}`);

    function handleFormError(error: unknown, details?: unknown) {
        handleError(error, details, formRef);
    }

    async function submitWarehouse(form$: { data?: Record<string, unknown> }) {
        ...
        await router.visit(saveAction.value === 'add-new' ? '/warehouse/add' : '/warehouse');
...
<template>
    <Head :title="`Edit ${formatedText('warehouse')}`" />
    ...
    <div class="product-form-page purchase-form-page">
        <WarehouseFormChrome
            ...
            @cancel="router.visit('/warehouse')"
            @save="save"
        >
            <div class="product-form product-form--sectioned purchase-form">
                <Loader v-if="!pageReady" message="Loading warehouse…" />
