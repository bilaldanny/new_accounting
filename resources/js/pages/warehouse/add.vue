<script setup lang="ts">
    ...
    defineOptions({
        layout: {
            title: 'New Warehouse',
            subtitle: 'Add a warehouse location for a branch',
            breadcrumbs: [
                {
                    title: 'Warehouse',
                    href: '/warehouse',
    ...
    const { formData, defaultFormData, emptyForm } = useWarehouses();
    ...
    async function submitWarehouse(form$: { data?: Record<string, unknown> }) {
        const response = await window.axios.post(API_ENDPOINTS.warehouses, {
            ...form$?.data,
        });
...
<template>
    <Head :title="`New ${formatedText('warehouse')}`" />
