<script setup lang="ts">
    ...
    defineOptions({
        layout: {
            title: 'Role Trash',
            subtitle: 'Restore or permanently delete removed roles',
            breadcrumbs: [
                {
    ...
    const debouncedGetTrashRoles = debounce((params) => {
        getTrashRoles(params);
    }, 300);
    ...
    const getData = async () => {
        try {
            ...
            await debouncedGetTrashRoles({ ...state.search });
    ...
    onMounted(() => {
        ...
        debouncedGetTrashRoles({ ...state.search });
    });

    const fetchAllRowsForExport = createTableExportAllRows(`${API_ENDPOINTS.roles}/trash`, () => state);
