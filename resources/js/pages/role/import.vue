<script setup lang="ts">
    ...
    const sampleColumns = ['id', 'branch_id', 'name', 'is_active'] as const;
    const fallbackSampleRows = [['', '', 'Sample Role', 1]];

    function mapRoleToSampleRow(role: Record<string, unknown>): (string | number)[] {
        return [
            normalizeImportExportValue(role.id),
            normalizeImportExportValue(role.branch_id),
            normalizeImportExportValue(role.name),
            normalizeImportBool(role.is_active),
        ];
    }
    ...
    async function downloadSampleFile() {
        ...
        try {
            await downloadImportTemplate({
                ...
                listApiUrl: API_ENDPOINTS.roles,
                listQuery: { sort_type: 'desc' },
                mapRow: mapRoleToSampleRow,
    ...
    async function submitImport() {
        ...
        try {
            ...
            if (rows.length === 0) {
                fileError.value = 'The selected file does not contain any role rows.';
...
<template>
    <ImportModalLayout
        ...
        file-input-id="role-import-file"
