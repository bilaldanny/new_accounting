<script setup lang="ts">
    ...
    defineOptions({
        layout: {
            title: 'Price List Trash',
            subtitle: 'Restore or permanently delete removed price lists',
            breadcrumbs: [
                {
                    title: 'Price List',
                    href: '/pricelist',
    ...
    const columns = [
        ...
        { key: 'brand_name', label: 'Brand', type: 'primary', responsive: ['xs', 'sm', 'md', 'lg'] },
        { key: 'branch_name', label: 'Branch', type: 'secondary', responsive: ['xs', 'sm', 'md', 'lg'], emptyDisplay: '-' },
        { key: 'date_label', label: 'Date', type: 'secondary', responsive: ['sm', 'md', 'lg'], emptyDisplay: '-' },
    ...
    const debouncedGetTrashPriceLists = debounce((params) => {
        getTrashPriceLists(params);
    }, 300);
    ...
    const getData = async () => {
        try {
            ...
            await debouncedGetTrashPriceLists({ ...state.search });
            ...
            console.error('Error fetching trash price lists:', error);
        }
    };
    ...
    onMounted(async () => {
        ...
        debouncedGetTrashPriceLists({ ...state.search });
    });
    ...
    const fetchAllRowsForExport = createTableExportAllRows(`${API_ENDPOINTS.priceLists}/trash`, () => state);
</script>
