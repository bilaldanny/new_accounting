<script setup lang="ts">
    ...
    const sampleColumns = ['id', 'branch_id', 'name', 'active'] as const;
    const fallbackSampleRows = [['', '', 'Sample Department', 1]];

    function mapDepartmentToSampleRow(department: Record<string, unknown>): (string | number)[] {
        return [
            normalizeImportExportValue(department.id),
            normalizeImportExportValue(department.branch_id),
            normalizeImportExportValue(department.name),
            normalizeImportBool(department.active),
        ];
    }
    ...
    async function downloadSampleFile() {
        ...
        try {
            await downloadImportTemplate({
                ...
                listApiUrl: API_ENDPOINTS.departments,
                listQuery: { sort_type: 'desc' },
                mapRow: mapDepartmentToSampleRow,
    ...
    async function submitImport() {
        ...
        try {
            ...
            if (rows.length === 0) {
                fileError.value = 'The selected file does not contain any department rows.';
...
<template>
    <ImportModalLayout
        ...
        file-input-id="department-import-file"
