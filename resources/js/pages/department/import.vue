<script setup lang="ts">
    ...
    const modalProps = defineProps({
        ...
        sampleFileName: { type: String, default: 'department-import-sample.xlsx' },
    });
    ...
    function mapDepartmentToSampleRow(department: Record<string, unknown>): (string | number)[] {
        return [
            normalizeExportValue(department.id),
            normalizeExportValue(department.company_id),
            normalizeExportValue(department.branch_id),
            normalizeExportValue(department.name),
            normalizeBool(department.active),
        ];
    }
    ...
    async function fetchAllDepartmentRows(): Promise<Record<string, unknown>[]> {
        const params = {
            status: 'all',
            search: '',
            sort_by: 'created_at',
        ...
        const { data: body } = await window.axios.get(API_ENDPOINTS.departments, { params });
        ...
        for (let page = 2; page <= lastPage; page++) {
            const { data: nextBody } = await window.axios.get(API_ENDPOINTS.departments, {
    ...
    async function downloadSampleFile() {
        ...
        try {
            const departments = await fetchAllDepartmentRows();
            const rows = departments.length > 0
                ? departments.map(mapDepartmentToSampleRow)
                : fallbackSampleRows;
    ...
    async function submitImport() {
        ...
        try {
            ...
            if (rows.length === 0) {
                Notify('The Excel file does not contain any department rows.', 'alert');

                return;
            }

            const response = await window.axios.post(API_ENDPOINTS.departmentImport, { rows });
...
<template>
    ...
    >
        <div class="row g-3">
                ...
                <div class="alert alert-light border d-flex flex-column flex-sm-row align-items-sm-center justify-content-between gap-3 mb-0">
                    <div>
                        ...
                        <div class="text-muted small">
                            Download the sample Excel file with existing department records when available. Leave the id column empty to add a new department, or keep the id to update an existing record.
                        </div>
                    </div>
            ...
            <div class="col-12">
                <label class="form-label" for="department-import-file">Excel file</label>
                <input
