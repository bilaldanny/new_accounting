<script setup lang="ts">
    ...
    defineOptions({
        layout: {
            ...
            subtitle: 'Restore or permanently delete removed departments',
            breadcrumbs: [
                {
    ...
    const debouncedGetTrashDepartments = debounce((params) => {
        getTrashDepartments(params);
    }, 300);
    ...
    const getData = async () => {
        try {
            ...
            await debouncedGetTrashDepartments({ ...state.search });
    ...
    onMounted(() => {
        ...
        debouncedGetTrashDepartments({ ...state.search });
    });

    const fetchAllRowsForExport = createTableExportAllRows(`${API_ENDPOINTS.departments}/trash`, () => state);
