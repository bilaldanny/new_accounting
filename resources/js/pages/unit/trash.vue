<script setup lang="ts">
    ...
    defineOptions({
        layout: {
            title: 'Unit Trash',
            subtitle: 'Restore or permanently delete removed units',
            breadcrumbs: [
                {
    ...
    const debouncedGetTrashUnits = debounce((params) => {
        getTrashUnits(params);
    }, 300);
    ...
    const getData = async () => {
        try {
            ...
            await debouncedGetTrashUnits({ ...state.search });
            ...
            console.error('Error fetching trash units:', error);
        }
    };
    ...
    onMounted(async () => {
        ...
        debouncedGetTrashUnits({ ...state.search });
    });
    ...
    const fetchAllRowsForExport = createTableExportAllRows(`${API_ENDPOINTS.units}/trash`, () => state);
