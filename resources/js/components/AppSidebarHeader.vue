<script setup lang="ts">
    ...
    const rawSetting = pageProps?.setting;
    const settingObject =
    ...
    const resolveAssetUrl = (value?: string | null, version?: number) => {
        ...
        if (normalizedRaw.startsWith('storage/')) {
            return appendVersionParam(`${normalizedBase}/${normalizedRaw}`, version);
        }

        if (normalizedRaw.startsWith('photos/')) {
            return appendVersionParam(`${normalizedBase}/storage/${normalizedRaw}`, version);
        }
    ...
    const settingLogo = settingObject?.system_logo_url || settingObject?.system_logo || '';
    const logoUrl = resolveAssetUrl(settingLogo) || `${appUrl.replace(/\/$/, '')}/assets/images/logo-light.png`
    const liveProfileImage = ref<{ profile_photo_path?: string; profile_photo_url?: string; profile_photo_version?: number }>({});
    const headerProfileImage = computed(() => {
        const raw = liveProfileImage.value?.profile_photo_url
            || liveProfileImage.value?.profile_photo_path
            || (user.value as any)?.profile_photo_url
            || (user.value as any)?.profile_image
            || (user.value as any)?.profile_photo_path
            || '';
        const version = liveProfileImage.value?.profile_photo_version;

        return resolveAssetUrl(raw, version) || `${appUrl.replace(/\/$/, '')}/assets/images/users/avatar-1.jpg`;
    });
