...
class ExpenseController extends Controller
{
     ...
     */
    protected function expenseFormRules(): array
    {
        return [
            'company_id' => Auth::user()?->hasRole('superadmin') ? 'required' : 'nullable',
            'branch_id' => 'bail|required',
            'voucher_type' => ['bail', 'required', Rule::in(TAccount::EXPENSE_VOUCHER_TYPES)],
    ...
    public function index(Request $request)
    {
        ...
        $expenses = $this->paginateSorted($query, $request);

        $expenses->getCollection()->transform(function (TAccount $expense) {
            return $expense->presentForIndex();
        });

        return response()->json(['data' => $expenses, 'trash_count' => 0]);
    }
    ...
    public function voucherNo(Request $request, LedgerJournal $ledgerJournal)
    {
        $request->validate([
            'company_id' => Auth::user()?->hasRole('superadmin') ? 'required' : 'nullable',
            'branch_id' => 'required',
            'type' => ['required', Rule::in(TAccount::EXPENSE_VOUCHER_TYPES)],
        ]);
    ...
    {
        $this->authorizeMenuPermission('/expense/add');

        $request->validate($this->expenseFormRules());
    ...
    {
        $expense = TAccount::findVisibleManualExpense((int) $id);

        if ($expense === null) {
            abort(404);
        }

        return response()->json($expense->presentForForm());
    }

    public function update(Request $request, $id)
    {
        $this->authorizeMenuPermission('/expense/:id/edit');
        ...
        DB::beginTransaction();
        try {
            TAccount::updateJournalEntry($request, (int) $id, 'expense');
            DB::commit();
        } catch (ValidationException $e) {
            DB::rollBack();
            throw $e;
    ...
    {
        if (deletepermission('/expense/delete')) {
            TAccount::deleteJournalEntry((int) $id, 'expense');
    ...
    {
        return $this->guardedBulkAction('/expense/delete', 'Successfully Deleted', function () use ($request) {
            $ids = TAccount::query()
                ->manualExpenses()
                ->visibleToCurrentUser()
    ...
    public function duplicate(Request $request)
    {
        ...
        DB::beginTransaction();
        try {
            TAccount::duplicateJournalEntry((int) $request->id, 'expense');
            DB::commit();
    ...
    private function assertLinesAreBalanced(Request $request): void
    {
        ...
        if (round($debit, 2) !== round($credit, 2)) {
            throw ValidationException::withMessages([
                'taccountdetails' => ['Expense is not balanced. Total debit must equal total credit.'],
            ]);
        }
    }
}
