...
class FundTransferController extends Controller
{
     ...
     */
    protected function fundTransferFormRules(): array
    {
        return [
            'company_id' => Auth::user()?->hasRole('superadmin') ? 'required' : 'nullable',
            'branch_id' => 'bail|required',
            'voucher_type' => ['bail', 'required', Rule::in(TAccount::FUND_TRANSFER_VOUCHER_TYPES)],
    ...
    public function index(Request $request)
    {
        ...
        $query = TAccount::query()
            ...
            ->when($search, function ($q) use ($search) {
                $q->where(function ($sub) use ($search) {
                    $sub->whereAny(['voucher_no', 'ref_no', 'comments'], 'like', "%{$search}%");
                });
            })
        ...
        $fundTransfers = $this->paginateSorted($query, $request);

        $fundTransfers->getCollection()->transform(function (TAccount $fundTransfer) {
            return $fundTransfer->presentForIndex();
        });

        return response()->json(['data' => $fundTransfers, 'trash_count' => 0]);
    }
    ...
    public function voucherNo(Request $request, LedgerJournal $ledgerJournal)
    {
        $request->validate([
            'company_id' => Auth::user()?->hasRole('superadmin') ? 'required' : 'nullable',
            'branch_id' => 'required',
            'type' => ['required', Rule::in(TAccount::FUND_TRANSFER_VOUCHER_TYPES)],
        ]);
    ...
    {
        $this->authorizeMenuPermission('/fundtransfer/add');

        $request->validate($this->fundTransferFormRules());
    ...
    {
        $fundTransfer = TAccount::findVisibleManualFundTransfer((int) $id);

        if ($fundTransfer === null) {
            abort(404);
        }

        return response()->json($fundTransfer->presentForForm());
    }

    public function update(Request $request, $id)
    {
        $this->authorizeMenuPermission('/fundtransfer/:id/edit');
        ...
        DB::beginTransaction();
        try {
            TAccount::updateJournalEntry($request, (int) $id, 'fundtransfer');
            DB::commit();
        } catch (ValidationException $e) {
            DB::rollBack();
            throw $e;
    ...
    {
        if (deletepermission('/fundtransfer/delete')) {
            TAccount::deleteJournalEntry((int) $id, 'fundtransfer');
    ...
    {
        return $this->guardedBulkAction('/fundtransfer/delete', 'Successfully Deleted', function () use ($request) {
            $ids = TAccount::query()
                ->manualFundTransfers()
                ->visibleToCurrentUser()
    ...
    public function duplicate(Request $request)
    {
        ...
        DB::beginTransaction();
        try {
            TAccount::duplicateJournalEntry((int) $request->id, 'fundtransfer');
            DB::commit();
    ...
    private function assertLinesAreBalanced(Request $request): void
    {
        ...
        if (round($debit, 2) !== round($credit, 2)) {
            throw ValidationException::withMessages([
                'taccountdetails' => ['Fund transfer is not balanced. Total debit must equal total credit.'],
            ]);
        }
    }
}
