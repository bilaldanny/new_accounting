...
class DepositController extends Controller
{
     ...
     */
    protected function depositFormRules(): array
    {
        return [
            'company_id' => Auth::user()?->hasRole('superadmin') ? 'required' : 'nullable',
            'branch_id' => 'bail|required',
            'voucher_type' => ['bail', 'required', Rule::in(TAccount::DEPOSIT_VOUCHER_TYPES)],
    ...
    public function index(Request $request)
    {
        ...
        $deposits = $this->paginateSorted($query, $request);

        $deposits->getCollection()->transform(function (TAccount $deposit) {
            return $deposit->presentForIndex();
        });

        return response()->json(['data' => $deposits, 'trash_count' => 0]);
    }
    ...
    public function voucherNo(Request $request, LedgerJournal $ledgerJournal)
    {
        $request->validate([
            'company_id' => Auth::user()?->hasRole('superadmin') ? 'required' : 'nullable',
            'branch_id' => 'required',
            'type' => ['required', Rule::in(TAccount::DEPOSIT_VOUCHER_TYPES)],
        ]);
    ...
    {
        $this->authorizeMenuPermission('/deposit/add');

        $request->validate($this->depositFormRules());
    ...
    {
        $deposit = TAccount::findVisibleManualDeposit((int) $id);

        if ($deposit === null) {
            abort(404);
        }

        return response()->json($deposit->presentForForm());
    }

    public function update(Request $request, $id)
    {
        $this->authorizeMenuPermission('/deposit/:id/edit');
        ...
        DB::beginTransaction();
        try {
            TAccount::updateJournalEntry($request, (int) $id, 'deposit');
            DB::commit();
        } catch (ValidationException $e) {
            DB::rollBack();
            throw $e;
    ...
    {
        if (deletepermission('/deposit/delete')) {
            TAccount::deleteJournalEntry((int) $id, 'deposit');
    ...
    {
        return $this->guardedBulkAction('/deposit/delete', 'Successfully Deleted', function () use ($request) {
            $ids = TAccount::query()
                ->manualDeposits()
                ->visibleToCurrentUser()
    ...
    public function duplicate(Request $request)
    {
        ...
        DB::beginTransaction();
        try {
            TAccount::duplicateJournalEntry((int) $request->id, 'deposit');
            DB::commit();
    ...
    private function assertLinesAreBalanced(Request $request): void
    {
        ...
        if (round($debit, 2) !== round($credit, 2)) {
            throw ValidationException::withMessages([
                'taccountdetails' => ['Deposit is not balanced. Total debit must equal total credit.'],
            ]);
        }
    }
}
