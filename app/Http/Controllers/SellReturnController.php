...
class SellReturnController extends Controller
{
     ...
     */
    protected function sellReturnRules(): array
    {
        return [
            ...
            'selllines.*.quantity_returned' => 'bail|required|numeric|min:0',
        ];
    }
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        return response()->json([
            'data' => Transaction::paginateSellReturns($filters),
            'trash_count' => Transaction::onlyTrashed()->sellReturns()->visibleToCurrentUser()->count(),
        ]);
    }
    ...
    public function eligibleSells(Request $request): JsonResponse
    {
        ...
        return response()->json(Transaction::eligibleReturnSells(
            $companyId,
    ...
    public function sellLines(int $id): JsonResponse
    {
        ...
        return response()->json([
            ...
            'selllines' => $sell->sellReturnLines(),
        ]);
    }

    public function store(Request $request): JsonResponse
    {
        $this->authorizeMenuPermission('/sell/return/add');

        $request->validate($this->sellReturnRules());

        DB::beginTransaction();
        try {
            Transaction::createSellReturn($request);
            DB::commit();
    ...
    {
        $note = Transaction::findVisibleSellReturn($id);

        if ($note === null) {
            abort(404);
        }

        return response()->json($note->presentSellReturn());
    }

    public function update(Request $request, int $id): JsonResponse
    {
        $this->authorizeMenuPermission('/sell/return/:id/edit');
        $request->validate([
            'selllines' => 'bail|required|array|min:1',
            'selllines.*.id' => 'bail|required',
            'selllines.*.quantity_returned' => 'bail|required|numeric|min:0',
        ...
        DB::beginTransaction();
        try {
            Transaction::updateSellReturn($request, $id);
            DB::commit();
        } catch (ValidationException $e) {
            DB::rollBack();
            throw $e;
    ...
    {
        if (deletepermission('/sell/return/delete')) {
            Transaction::deleteSellReturn($id);
    ...
    {
        return $this->guardedBulkAction('/sell/return/delete', 'Successfully Deleted', function () use ($request) {
            ...
            foreach ($ids as $id) {
                Transaction::deleteSellReturn((int) $id);
            }
        });
    }
    ...
    {
        if (deletepermission('/sell/return/restore')) {
            DB::beginTransaction();
            try {
                $ids = Transaction::query()
                    ->onlyTrashed()
