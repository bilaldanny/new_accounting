...
class PaymentController extends Controller
{
    ...
    protected function paymentFormRules(): array
    {
        return [
            ...
            'voucher_type' => ['bail', 'required', Rule::in(TAccount::PAYMENT_VOUCHER_TYPES)],
    ...
    public function index(Request $request)
    {
        ...
        $payments->getCollection()->transform(function (TAccount $payment) {
            return $payment->presentForIndex();
        });
    ...
    public function voucherNo(Request $request, LedgerJournal $ledgerJournal)
    {
        $request->validate([
            'company_id' => Auth::user()?->hasRole('superadmin') ? 'required' : 'nullable',
            'branch_id' => 'required',
            'type' => ['required', Rule::in(TAccount::PAYMENT_VOUCHER_TYPES)],
        ]);
    ...
    {
        $payment = TAccount::findVisibleManualPayment((int) $id);

        if ($payment === null) {
            abort(404);
        }

        return response()->json($payment->presentForForm());
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        DB::beginTransaction();
        try {
            TAccount::updateJournalEntry($request, (int) $id, 'payment');
            DB::commit();
        } catch (ValidationException $e) {
            DB::rollBack();
            throw $e;
    ...
    public function duplicate(Request $request)
    {
        ...
        DB::beginTransaction();
        try {
            TAccount::duplicateJournalEntry((int) $request->id, 'payment');
            DB::commit();
    ...
    private function assertLinesAreBalanced(Request $request): void
    {
        ...
        if (round($debit, 2) !== round($credit, 2)) {
            throw ValidationException::withMessages([
                'taccountdetails' => ['Payment is not balanced. Total debit must equal total credit.'],
            ]);
        }
    }
}
