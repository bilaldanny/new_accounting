 ...
 * created with) the Cash Collection page and its hidden rows (add, edit, delete, view, restore, export, complete, cancel):
 * 9 `permissions` rows.
 *
 * 2026_09_21_260100_add_cashcollection_menu only adds the menu rows; roles get `permissions` rows through the Role Permission
 * screen, so without this the page is not in the company admin's sidebar. Superadmin is always allowed
 * and other roles are left alone.
 *
 * Follows 2026_09_21_190000_grant_companyadmin_transporter_commission_agent_menus: idempotent (an
 * existing row is re-enabled, a missing one added), a menu row that does not exist is skipped, and
 * `down()` removes only these rows.
 */
