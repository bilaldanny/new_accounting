 ...
 * created with) the Transporter and Commission Agent pages and their hidden rows (add, edit, delete,
 * view, restore, export): 14 `permissions` rows.
 *
 * 2026_09_21_180300 / 180400 only add the menu rows; roles get `permissions` rows through the Role
 * Permission screen, so without this the two pages are not in the company admin's sidebar. Superadmin is
 * always allowed and other roles are left alone.
 *
 * Follows 2026_09_21_170000_grant_companyadmin_report_menu_permissions: idempotent (an existing row is
 * re-enabled, a missing one added), a menu row that does not exist is skipped, and `down()` removes only
 * these rows.
 */
return new class extends Migration
{
    ...
    private const PATHS = [
        ...
        '/commissionagent',
        '/commissionagent/add',
        '/commissionagent/:id/edit',
        '/commissionagent/delete',
        '/commissionagent/:id/view',
        '/commissionagent/restore',
        '/commissionagent/export',
    ];
