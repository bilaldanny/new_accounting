...
/**
 * The Receipt Printer Settings page in the Company Settings group, with the hidden permission rows its controller and pages
 * check: update.
 *
 * The Company Settings group is a legacy row that only exists in live data, so it is found through the Company Settings
 * page (`/business/settings`) and the migration does nothing without it. Permissions are not granted here: roles
 * get the rows from the Role Permission screen (see the companyadmin grant migration for the default role).
 */
return new class extends Migration
{
    private const PATH = '/receipt/settings';
    ...
    public function up(): void
    {
        ...
        $groupId = DB::table('menus')->where('route_path', '/business/settings')->value('parent_id');
        ...
        $pageId = DB::table('menus')->insertGetId($this->row([
            'parent_id' => $groupId,
            'name' => 'Receipt Printer Settings',
            'icon' => 'Cog',
            'route_name' => 'receipt.settings',
        ...
        $children = [
            ['Update Receipt Printer Settings', 'receipt.settings.update', '/receipt/settings/update'],
        ];
