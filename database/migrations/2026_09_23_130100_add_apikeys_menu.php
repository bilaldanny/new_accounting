...
/**
 * The API Keys page in the Settings group, with the hidden permission rows its controller checks: add (create a
 * key) and delete (revoke one). The Settings group is a legacy row that only exists in live data, so it is found
 * through the Settings page (/software/setting) and the migration does nothing without it. Permissions are not
 * granted here (see the companyadmin grant migration that follows).
 */
return new class extends Migration
{
    private const PATH = '/apikeys';
    ...
    public function up(): void
    {
        ...
        $children = [
            ['Create API Key', 'apikeys.add', '/apikeys/add'],
            ['Revoke API Key', 'apikeys.delete', '/apikeys/delete'],
        ];
